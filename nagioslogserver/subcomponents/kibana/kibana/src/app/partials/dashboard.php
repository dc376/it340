...
<nil ng-repeat="pulldown in dashboard.current.pulldowns" ng-controller="PulldownCtrl" ng-show="pulldown.enable">
  <div class="top-row-close pointer pull-left" ng-class="pulldownTabStyle($index)" ng-click="toggle_pulldown(pulldown);dismiss();" bs-tooltip="'<?php echo _('Toggle'); ?> '+lang(pulldown.type)" data-placement="bottom">
    <span class="small">{{lang(pulldown.type)}}</span>
    <i class="small" ng-class="{'fa-caret-left':pulldown.collapse,'fa-caret-right':!pulldown.collapse}"></i>
    <i class="small fa-star" ng-show="row.notice && pulldown.collapse"></i>
  </div>
    ...
    <div class="kibana-container container">
        ...
        <div class="row-control">
          <div class="" style="padding:0px;margin:0px;position:relative;">
            ...
            <div class="row-close" ng-show="row.collapse" data-placement="bottom" >
              <span class="row-button bgWarning" config-modal="app/partials/roweditor.php" kbn-model="row" class="pointer">
                <i bs-tooltip="'Configure row'" data-placement="right" ng-show="row.editable" class="fa-cog pointer"></i>
              </span>
              <span class="row-button bgPrimary" ng-click="toggle_row(row)" ng-show="row.collapsable">
                <i bs-tooltip="'Expand row'" data-placement="right" ng-show="row.collapsable" class="fa-caret-left pointer" ></i>
              </span>
            ...
            <div style="text-align:center" class="row-open" ng-show="!row.collapse">
              <div ng-show="row.collapsable" class='row-tab bgPrimary' ng-click="toggle_row(row)">
                <i bs-tooltip="'<?php echo _('Collapse row'); ?>'" data-placement="right"  class="fa-caret-right" ></i>
                <br>
              </div>
              <div config-modal="app/partials/roweditor.php" kbn-model="row" class='row-tab bgWarning' ng-show="row.editable">
                <i bs-tooltip="'<?php echo _('Configure row'); ?>'" data-placement="right"  class="fa-cog pointer"></i>
                <br>
              </div>
              <div class="bgSuccess row-tab" ng-click="editor.index = 2" config-modal="app/partials/roweditor.php" kbn-model="row" ng-show="row.editable">
                <i class="fa-plus-sign" bs-tooltip="'<?php echo _('Add Panel'); ?>'" data-placement="right"></i>
                <br>
              </div>
          ...
          <div style="padding-top:0px" ng-if="!row.collapse">
            ...
            <span config-modal="app/partials/roweditor.php" kbn-model="row" ng-show="row.editable && !dashboard.panelDragging">
      ...
      <div class="row-fluid" ng-show='dashboard.current.editable'>
        <div class="span12" style="text-align:right;">
          <span class="pointer btn btn-mini row-add-button" config-modal="app/partials/dasheditor.php" kbn-model="dashboard">
            <span ng-click="editor.index = 2"><i class="fa-plus-sign"></i> ADD A ROW</span>
          </span>
        </div>
      </div>
