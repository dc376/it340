...
// Set a title
dashboard.title = 'Nagios Log Server Search';
...
// Someone sent a long services string...
if (!_.isUndefined(ARGS.services)) {
  dashboard.services = JSON.parse(ARGS.services);
  
  // lets overwrite the timeframe with what was passed if ARGS.from is present
  if (!_.isUndefined(ARGS.from)) {
      dashboard.services.filter.list[0] = {
              from: "now-"+(ARGS.from||_d_timespan),
              to: "now",
              field: ARGS.timefield||"@timestamp",
              type: "time",
    ...
    // lets overwrite the timeframe with what was passed if ARGS.from is present
  if (!_.isUndefined(ARGS.s_from) && !_.isUndefined(ARGS.s_to)) {
      dashboard.services.filter.list[0] = {
              from: (ARGS.s_from),
              to: (ARGS.s_to),
              field: ARGS.timefield||"@timestamp",
...
dashboard.rows[1].panels = [
  {
    ...
    size: 50,
    pages: 5,
    fields: !_.isUndefined(ARGS.fields) ? ARGS.fields.split(',') : ["@timestamp","host","type","message"],
