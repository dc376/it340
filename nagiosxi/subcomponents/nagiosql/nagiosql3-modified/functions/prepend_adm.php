...
//clean all input variables from SQL injection
...
if ($myDBClass->error == true) {
  echo _('Error while connecting to database:')."<br>".$myDBClass->strDBError."<br>";
  exit;
}
...
if (!isset($loc)) {
 echo _("Error in setting the correct locale, please report this error with the associated output of  'locale -a' to bugs@nagiosql.org")."<br>";
}
...
if (isset($preUsername)) {
  $strSQL    = "SELECT * FROM `tbl_user` WHERE `username`='".mysql_real_escape_string($preUsername)."' AND `password`=MD5('".mysql_real_escape_string($prePassword)."') AND `active`='1'";
  ...
  if ($booReturn == false) {
    ...
    $myDataClass->writeLog(_('Login successfull'));
    $_SESSION['strLoginMessage'] = "";
  } else {
    $_SESSION['strLoginMessage'] = _('Login failed!');
    $myDataClass->writeLog(_('Login failed!')." - Username: ".htmlentities(mysql_real_escape_string($preUsername),ENT_QUOTES));
    $preNoMain = 0;
  }
}
//
...
if (($_SESSION['username'] != "") && (!isset($preNoLogCheck) || ($preNoLogCheck == 0))) {
  ...
  if ($booReturn == false) {
    ...
    if (mktime() - $_SESSION['timestamp'] > $SETS['security']['logofftime']) {
      ...
      $myDataClass->writeLog(_('Session timeout reached - Seconds:')." ".(mktime() - $_SESSION['timestamp']." - User: ".$_SESSION['username']));
      $_SESSION['username'] = "";
      ...
      if (isset($preAccess) && ($preAccess == 1) && ($intSub != 0)) {
        ...
        if ($intResult != 0) {
          $myDataClass->writeLog(_('Restricted site accessed:')." ".$_SERVER['PHP_SELF']);
    ...
    // Neues Login erzwingen
    $myDataClass->writeLog(_('User not found in database'));
    header("Location: ".$SETS['path']['protocol']."://".$_SERVER['HTTP_HOST'].$SETS['path']['root']."index.php");
  }
...
if (!isset($preNoMain) || ($preNoMain == 0) && (!isset($preUsername))) {
  ...
  $maintp->setVariable("ADMIN","<a href=\"".$SETS['path']['root']."admin.php\" class=\"top-link\">"._('Administration')."</a>");
  ...
  if (isset($intMain) && (isset($intMenu) && ($intMenu != 1))) $maintp->setVariable("POSITION",$myVisClass->getPosition($intMain,$intSub,_('Admin')));
  $maintp->parse("header");
  ...
  // ===================
  if ($_SESSION['username'] != "") {
    $maintp->setVariable("LOGIN_INFO",_('Logged in:')." ".$_SESSION['username']);
