...
class nagdata {
  ...
  function dataInsert($strSQL,&$intDataID) {
    ...
    if ($booReturn == true) {
      // Erfolgreich
      $this->strDBMessage = _('Data were successfully inserted to the data base!');
      return(0);
    } else {
      // Misserfolg
      $this->strDBMessage = _('Error while inserting the data to the data base:')."<br>".$this->myDBClass->strDBError;
      return(1);
    }
  }
    ///////////////////////////////////////////////////////////////////////////////////////////
  ...
  function dataDeleteFull($strTableName,$intDataId = 0,$intForce = 0) {
    ...
    if ($intDataCount != 0) {
        ...
        if (isset($_POST[$strChbName]) && ($_POST[$strChbName] == "on")) {
          ...
          if ($this->infoRelation($strTableName,$elem['id'],"id") != 0) {
            ...
            if ($strTableName == "tbl_host") {
              ...
              if ($intReturn == 0) {
                $intFileRemoved = 1;
                $strFileMessage .=  "<br>"._('The assigned, no longer used configuration files were deleted successfully!');
                $this->writeLog(_('Host file deleted:')." ".$strHost.".cfg");
              } else {
                $intFileRemoved = 2;
      ...
      // Mitteilungen ausgeben
      if ($intDeleteCount == 0) {
        $this->strDBMessage .= _('No data deleted. Probably the dataset does not exist, is protected from deletion or has relations to other configurations which cannot be deleted. Use the "info" function for detailed informations about relations!');
        return(1);
      } else {
        $this->strDBMessage .= _('Dataset successfully deleted. Affected rows:')." ".$intDeleteCount;
        $this->writeLog(_('Delete data from table:')." $strTableName "._('- with affected rows:')." ".$this->myDBClass->intAffectedRows);
      ...
      $this->strDBMessage .= _('No data deleted. Probably the dataset does not exist or it is protected from deletion.');
      return(1);
    }
  }
    ///////////////////////////////////////////////////////////////////////////////////////////
  ...
  function infoRelation($strTable,$intMasterId,$strMasterfield,$intReporting=0) {
    ...
    if ($intReturn == 1) {
      ...
      $this->strDBMessage = _("<span style=\"color:#0000FF;\">Relation information for <b>").$strName._("</b> of table <b>").$strTable.":</b><br></span>\n";
      foreach ($arrRelations AS $elem) {
        // Flags auflösen
          ...
          if ($arrFlags[3] == 1) {
            foreach ($arrData AS $data) {
			  ...
			  if (($arrFlags[0] == 1) && ($strTarget != "-")) {
                ...
                if ($intDCCount > 1) {
                  $this->strDBMessage .= _("Relation to <b>").$elem['target']._("</b>, entry <b>").$strTarget._("</b> - deletion <b>possible</b>")."<br>\n";
                } else {
                  $this->strDBMessage .= _("Relation to <b>").$elem['target']._("</b>, entry <b>").$strTarget." - </b><span style=\"color:#FF0000;\">"._("deletion <b>not possible</b>")."</span><br>\n";
                  $intDeletion = 1;
                }
              } else if ($strTarget != "-") {
                $this->strDBMessage .= _("Relation to <b>").$elem['target']._("</b>, entry <b>").$strTarget._("</b> - deletion <b>possible</b>")."<br>\n";
            ...
            foreach ($arrDataCheck AS $data) {
              ...
              if ($arrFlags[0] == 1) {
                $this->strDBMessage .= _("Relation to <b>").$elem['tableName']._("</b>, entry <b>").$strTarget." - </b><span style=\"color:#FF0000;\">"._("deletion <b>not possible</b>")."</span><br>\n";
                $intDeletion = 1;
              } else {
                $this->strDBMessage .= _("Relation to <b>").$elem['tableName']._("</b>, entry <b>").$strTarget._("</b> - deletion <b>possible</b>")."<br>\n";
              }
            }
          }
        }
