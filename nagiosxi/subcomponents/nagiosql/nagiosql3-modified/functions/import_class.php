    ...
    if ($intMethod == 1) {
      ...
      if ((!$conn_id) || (!$login_result)) {
        ...
        if (!ftp_get($conn_id,$this->arrSettings['path']['tempdir']."/nagiosql_import_temp.dat",$strFileName,FTP_ASCII)) {
          $this->strDBMessage = _('Cannot open the configuration file (FTP connection failed)!');
          ftp_close($conn_id);
          return(1);
        }
        $intRemoveTmp   = 1;
...
//      if ($strTemp == "") 
    ...
    if (($intExists != 0) && ($intOverwrite == 0)) {
      $this->strMessage .= _('Entry')." ".$strKeyField."::".$arrImportData[$strKeyField]['value']._('inside')." "._('exists and were not overwritten')."<br>";
      return(2);
    }
   ...
   // Statement of the values
    ...
    //handle error if there's a problem 
    if ($booResult != true) 
    {
      $this->strDBMessage = $this->myDBClass->strDBError;
      if ($strKeyField == "")
      if ($strKeyField != "") $this->strMessage .= _('Entry')." ".$strKey."::".$arrImportData[$strKeyField]['value']." "._('inside')." ".$strTable." "._('could not be inserted:')." ".mysql_error()."<br>";
      if ($strKeyField == "") $this->strMessage .= _('Entry')." ".$strTemp1."::".$strTemp2._('inside')." ".$strTable." ".$strTable." "._('could not be inserted:')." ".mysql_error()."<br>";
      return(1);
    } 
    else ///////////////SUCCESS 
    {
        ...
        foreach ($arrImportRelations AS $elem) 
        {
          foreach ($arrRelations AS $reldata) 
          {
            ...
            if ($reldata['fieldName'] == $elem['key']) 
            {
              ...
              } else if ($reldata['type'] == 2) { //service escalations use writeRelation_2 
  ...
  function writeRelation_2($strKey,$strValue,$intDataId,$strDataTable,$arrRelData) {
    ...
    if (strtolower(trim($strValue)) == "null") 
    {
      ...
      foreach ($arrValues AS $elem) 
      {
        ...
        ///////////////////////////////XI FIX FOR SERVICE ESCALATIONS/DEPENCENCIES /////////////////////////////////////////////////////
        ///////run a different query and assign to a matched service description, prevents ghost relationship entry that can never be fixed  
        if(($strDataTable == "tbl_servicedependency") || ($strDataTable == "tbl_serviceescalation") && $strId=='') {        	 
        	$strSQL = "SELECT `id` FROM `tbl_service` WHERE `active`='1' 
      					AND `config_id`='".$this->intDomainId."' AND `$strKey`='$strValue' ORDER BY `id` LIMIT 1";       	        	
        	$strId  = $this->myDBClass->getFieldData($strSQL);	
        }	
		////////////////////////////////////END XI FIX/////////////////////////////////////////////////        
		...
		//create a blank entry if we can't find anything 
