...
<div id="installmain">
  <div id="installmain_content">
    <h1>NagiosQL <?php echo _($_SESSION['InstallType']). ": ". _("Finishing Setup"); ?></h1>
    <table width="100%">
      <?php
      switch ($_SESSION['InstallType']) {
        case "Installation":
          ...
          if ($_SESSION['db_drop'] == 0) {
            ...
            if ($selectDB) {
              echo "<tr>\n";
              echo "<td colspan='2' class='red'>"._("Database already exists and drop database was not selected, please correct or manage manually").".</td>\n";
              echo "</tr>\n";
              $intError=1;
            }
            mysql_close($link);
          ...
          if ($intError != 1) {
            echo "<tr>\n";
            echo "<td>"._("MySQL server connection (privileged user)")."</td>\n";
            ...
            if ($errmsg != "") {
              ...
              echo "<td class='green'>"._("passed")."</td></tr>\n";
              echo "<tr>\n";
              echo "<td>"._("MySQL server version")."</td>\n";
              ...
              if (mysql_error() == "") {
                ...
                echo "<tr>\n";
                echo "<td>"._("MySQL server support")."</td>\n";
                if ($setMySQLVersion != 0) {
                  echo "<td class='green'>"._("supported")."</td></tr>\n";
                } else {
                  echo "<td class='red'>"._("not supported")."</td></tr>\n";
                  $intError = 1;
                }
              } else {
                 echo "<td class='red'>"._("failed")."</td></tr>\n";
                 $intError = 1;
              }
            }
          }
          ...
          if ($intError != 1) {
            echo "<tr>\n";
            echo "<td>"._("Creating new database")." ".htmlspecialchars($_SESSION['db_name'])."</td>\n";
            ...
            if (file_exists($strFile) AND is_readable($strFile)) {
              ...
              if ($errmsg == "") {
                ...
                echo "<td class='red'>"._("Could not access")." ".$strFile."</td>\n";
                $intError = 1;
            }
            echo "</tr>\n";
          }
          ...
          if ($intError != 1) {
            echo "<tr>\n";
            echo "<td>"._("Create NagiosQL MySQL User")."</td>\n";
          ...
          if ($intError != 1) {
            echo "<tr>\n";
            echo "<td>"._("Update MySQL Permissions")."</td>\n";
          ...
          if ($intError != 1) {
            echo "<tr>\n";
            echo "<td>"._("Reloading MySQL User Table")."</td>\n";
          ...
          if ($intError != 1) {
            echo "<tr>\n";
            echo "<td>"._("Testing database connection to")." ".htmlspecialchars($_SESSION['db_name'])."</td>\n";
            ...
            if ($selectDB) {
              echo "<td class='green'>"._("passed")."</td></tr>\n";
              echo "<tr>\n";
              echo "<td>"._("Writing global settings to database")."</td>\n";
              if (writeSettingsDB($errmsg)) {
                echo "<td class='green'>"._("done")."</td>\n";
              } else {
                echo "<td class='red'>"._("failed")."</td>\n";
                echo "</tr>\n";
              ...
              echo "</tr>\n";
              echo "<tr>\n";
              echo "<td>"._("Writing database configuration to settings.php")."</td>\n";
              ...
              echo "<td class='red'>"._("error")."</td></tr>\n";
          ...
          if ($intError != 1) {
            echo "<tr>\n";
            echo "<td>"._("Set initial NagiosQL Administrator")."</td>\n";
          ...
          if ($intError != 1 && $_SESSION['sampleData'] == 1) {
            echo "<tr>\n";
            echo "<td>"._("Import Nagios sample data")."</td>\n";
        ...
        case "Update":
          ...
          echo "<tr>\n";
          echo "<td>"._("Installed NagiosQL Version")."</td>\n";
          ...
          if ($result) {
            ...
            if ($strCurrentVersion != "") {
              echo "<td class='red'>".$strCurrentVersion." "._("is not supported!")."</td>\n";
              $intError=1;
            } else {
              echo "<td class='red'>".$errmsg."</td>\n";
              $intError=1;
          ...
          if ($intError != 1) {
            if ( $_SESSION['from_db_name'] != $_SESSION['db_name'] OR $_SESSION['from_db_server'] != $_SESSION['db_server'] OR $_SESSION['from_db_port'] != $_SESSION['db_port'] ) {
              echo "<tr>\n<td>"._("Connect to source server")." ".htmlspecialchars($_SESSION['from_db_server'])."</td>";
              if (!preg_match('/^((1?\d{1,2}|2[0-4]\d|25[0-5])\.){3}(1?\d{1,2}|2[0-4]\d|25[0-5]){1}$/',$_SESSION['from_db_server'])) {
                ...
                if ($ip_from == $_SESSION['from_db_server']) {
                  $intError=1;
                  echo "<td class='red'>"._("Server not found!")."</td>\n</tr>\n";
                }
              }
              if ($intError != 1) {
                ...
                if ($errmsg != "") {
                  ...
                  echo "<td class='green'>"._("passed")."</td>\n</tr>\n";
                  echo "<tr>\n<td>"._("Connect to target server")." ".htmlspecialchars($_SESSION['db_server'])."</td>";
                  ...
                  if ($intError != 1) {
                    ...
                    if ($errmsg != "") {
                      ...
                      echo "<tr>\n";
                      echo "<td>"._("Copying old database to new database")." <a href='javascript:Klappen(3)'><img src='images/plus.png' id='SwPic3' border='0' /></a>\n";
                      echo "<!-- DIV Container for copy table details -->\n";
                      echo "<div id='SwTxt3' style='display: none;'>\n";
                        echo "<table width='100%' cellpadding='0' cellspacing='0'>\n";
                        while($row = mysql_fetch_row($sth)) {
                          echo "<tr>\n";
                          echo "<td>"._("Copy table")." ".$row[0]." "._("to")." ".$_SESSION['db_name']."</td>\n";
                          do_mysql_table($row[0],$dbfrom,$dbto,$errmsg);
                          if ($errmsg != "") {
                            echo "<td width='20%' class='red' align='left'>".$errmsg."</td>\n";
                            $intError=1;
                          } else {
                            echo "<td width='20%' class='green' align='left'>"._("done")."</td>\n";
                          }
                          echo "</tr>\n";
                        }
                        echo "</table>\n";
                      ...
                      echo "</td>\n";
                      if ($intError!=1) {
                        echo "<td class='green' valign='top'>"._("done")."</td>\n";
                      } else {
                        echo "<td class='red' valign='top'>"._("failed")."</td>\n";
                      }
                      echo "</tr>\n";
                    }
                  }
          ...
          if ($intError != 1) {
            while ($strCurrentVersion != $_SESSION['version'] AND $errmsg == "") {
              echo "<tr>\n";
              echo "<td>"._("Upgrading from version")." ".$strCurrentVersion." "._("to")."</td>\n";
    ...
    echo "<br />\n";
    if ($intError != 1) {
      echo "<span class='red'>"._("Please delete the install directory to continue!")."</span><br /><br />\n";
      echo "<div id=\"install-next\">\n";
      echo "<a href='../index.php'><img src='images/next.png' alt='finish' border='0' /></a><br />"._("Finish")."\n";
      echo "</div>\n";
    } else {
      echo "<div id=\"install-back\">\n";
      echo "<form action='' method='post'>\n";
      echo "<input type='hidden' name='step' value='2' />\n";
      echo "<input type='image' src='images/previous.png' value='Submit' alt='Submit' /><br />"._("Back")."\n";
      echo "</form>\n";
      echo "</div>\n";
    }
    ?>
