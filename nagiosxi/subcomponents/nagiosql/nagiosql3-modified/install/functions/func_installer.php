...
function mysql_install_db($dbname, $dbsqlfile,&$errmsg) {
  ...
  if(!mysql_select_db($dbname)) {
    ...
    if(!$result) {
      $errmsg = _("Could not create")." [".$dbname."] "._("db in mysql");
      return false;
    }
    $result = mysql_select_db($dbname);
  }
  if(!$result) {
    $errmsg = _("Could not select")." [".$dbname."] "._("database in mysql");
    return false;
  }
...
function writeSettingsFile(&$errmsg) {
  ...
  if ($filSet) {
    ...
    $errmsg=_("Could not open settings.php in config directory for writing!");
    return false;
  }
}
...
function updateQL($strCurrentVersion, $dbhost, $dbport, $dbprivuser, $dbprivpass, $dbname, &$errmsg) {
  ...
  if (isset($strFile) AND file_exists($strFile) AND is_readable($strFile)) {
    ...
    if ($link) {
     ...
     if ($errmsg == "") $errmsg=_("Could not access")." ".$strFile;
     $result=false;
  }
  return $result;
}
...
function db_connect($host,$port,$user,$pass,$db,$charset,&$errmsg) {
  ...
  if (!$dbh) {
    $errmsg=_("Error").": "._("Cannot connect to the database.")." "._("MySQL Error").": ".mysql_error();
  }
  if (($dbh) AND $db != "") {
    $create=mysql_query("CREATE DATABASE IF NOT EXISTS `".$db."`");
    if (!$create) {
      $errmsg=_("Error").": ".mysql_error();
    } else {
      $res=mysql_select_db($db, $dbh);
      if (!$res) {
         $errmsg=_("Error").": "._("Cannot select the database.")." "._("MySQL Error").": ".mysql_error();
      }else{
         $res=mysql_query("SET CHARSET ".$charset);
         if (!$res) $errmsg=_("Error").": "._("Cannot set")." CHARSET. "._("MySQL Error").": ".mysql_error();
         $res=mysql_query("SET NAMES ".$charset);
         if (!$res) $errmsg=_("Error").": "._("Cannot set")." CHARSET NAMES. "._("MySQL Error").": ".mysql_error();
      }
    }
  }
  return $dbh;
