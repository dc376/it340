...
if (($chkModus == "insert") || ($chkModus == "modify")) {
  ...
  if (($chkInsName != "") && ($chkInsAlias != "")) {
    ...
    if ($intInsert == 1) {
      ...
      if ($chkModus  == "insert")   $myDataClass->writeLog(_('New time period inserted:')." ".$chkInsName);
      if ($chkModus  == "modify")   $myDataClass->writeLog(_('Time period modified:')." ".$chkInsName);
      //
...
// Content einbinden
// =================
$conttp->setVariable("TITLE",_('Timeperiod definitions'));
$conttp->parse("header");
$conttp->show("header");
//
// Eingabeformular
...
if ($chkModus == "add") {
  ...
  $conttp->setVariable("FIELDNAME",_('Exclude data'));
  $conttp->setVariable("LANG_INSERT_ALL_TIMERANGE",_('Please insert a time definition and a time range'));
...
if ($chkModus == "display") {
  ...
  $mastertp->setVariable("FIELD_1",_('Time period'));
