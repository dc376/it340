...
if (($chkModus == "insert") || ($chkModus == "modify")) {
  ...
  if ((($chkInsPwd1 === $chkInsPwd2) && (strlen($chkInsPwd1) > 5)) || (($chkModus == "modify") && ($chkInsPwd1 == ""))) {
    ...
    if (($chkInsName != "") && ($chkInsAlias != "")) {
      ...
      if ($chkModus  == "insert")   $myDataClass->writeLog(_('A new user added:')." ".$chkInsName);
      if ($chkModus  == "modify")   $myDataClass->writeLog(_('User modified:')." ".$chkInsName);
    } else {
  ...
  if ($chkHidName != "Admin") {
    ...
    $myDataClass->strDBMessage = _("Admin can't be deleted");
  }
  $chkModus  = "display";
  ...
  if ($booReturn == false) $strMessage .= _('Error while selecting data from database:')."<br>".$myDataClass->strDBError."<br>";
  $chkModus      = "add";
}
...
// Content einbinden
// =================
$conttp->setVariable("TITLE",_('User administration'));
$conttp->parse("header");
$conttp->show("header");
//
// Eingabeformular
...
if ($chkModus == "add") {
  ...
  $conttp->setVariable("FILL_PASSWD_NOT_EQUAL",_('The passwords are not equal!'));
  $conttp->setVariable("FILL_PASSWORD",_('Please fill in the password'));
  $conttp->setVariable("FILL_PWDSHORT",_('The password is too short - use at least 6 characters!'));
  $conttp->setVariable("LANG_WEBSERVER_AUTH",_('Webserver authentification'));
...
if ($chkModus == "display") {
  ...
  $mastertp->setVariable("FIELD_1",_('Username'));
  $mastertp->setVariable("FIELD_2",_('Description'));
  $mastertp->setVariable("DELETE",_('Delete'));
  $mastertp->setVariable("LIMIT",$chkLimit);
  $mastertp->setVariable("DUPLICATE",_('Copy'));
  $mastertp->setVariable("ACTION_MODIFY",$_SERVER['PHP_SELF']);
  $mastertp->setVariable("LANG_DELETESINGLE",_('Do you really want to delete this database entry:'));
  $mastertp->setVariable("LANG_DELETEOK",_('Do you really want to delete all marked entries?'));
