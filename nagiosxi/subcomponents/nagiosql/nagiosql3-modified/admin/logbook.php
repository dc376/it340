...
// Content einbinden
// =================
$conttp->setVariable("TITLE",_('View logbook'));
foreach($arrDescription AS $elem) {
  $conttp->setVariable($elem['name'],$elem['string']);
}
$conttp->setVariable("LANG_ENTRIES_BEFORE",_('Delete logentries between:'));
$conttp->setVariable("LOCALE",$SETS['data']['locale']);
$conttp->setVariable("LANG_SELECT_DATE",_('Please at least fill in a start or a stop time'));
$conttp->setVariable("LANG_DELETELOG",_('Do you really want to delete all log entries between the selected dates?'));
$conttp->setVariable("DAT_SEARCH",$chkSearch);
// Legende einblenden
if ($chkFromLine > 1) {
  $intPrevNumber = $chkFromLine - 20;
  $conttp->setVariable("LANG_PREVIOUS", "<a href=\"".$_SERVER['PHP_SELF']."?from_line=".$intPrevNumber."\"><< "._('previous 20 entries')."</a>");
} else {
  $conttp->setVariable("LANG_PREVIOUS", "");
}
if ($chkFromLine < $intNumRows-20) {
  $intNextNumber = $chkFromLine + 20;
  $conttp->setVariable("LANG_NEXT", "<a href=\"".$_SERVER['PHP_SELF']."?from_line=".$intNextNumber."\">"._('next 20 entries')." >></a>");
} else {
