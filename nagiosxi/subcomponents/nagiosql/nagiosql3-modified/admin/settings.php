...
if ( (isset($_POST)) AND (isset($_POST['selLanguage']))) {
  ...
  $booReturn = $myDBClass->insertData($strSQL);
  if ( $booReturn == false ) $writingmsg = _("An error occured while writing settings to database")."<br>".$myDBClass->strDBError;
  ...
  if ($filSet) {
    ...
    if (!isset($loc)) {
      $strMessage = _("Error in setting the correct locale, please report this error with the associated output of  'locale -a' to bugs@nagiosql.org");
    }
    ...
    textdomain($selLanguage);
		$writingmsg = _("Settings were changed");
  } else {
    $writingmsg = _("An error occured while writing settings.php, please check permissions!");
  }
}
//
// Menu aufbauen
...
// Content einbinden
// =================
$conttp->setVariable("TITLE",_('Configure Settings'));
$conttp->parse("header");
$conttp->show("header");
...
$conttp->setVariable("LANG_DESCRIPTION",_('Change your current NagiosQL settings (e.g. Database user, Language).'));
//
// path settings
//
$conttp->setVariable("PATH",_('Path'));
$conttp->setVariable("ROOT_NAME",_('Application root path'));
$conttp->setVariable("ROOT_VALUE",$txtRootPath);
$conttp->setVariable("PHYSICAL_NAME",_('Application base path'));
$conttp->setVariable("PHYSICAL_VALUE",$txtBasePath);
$conttp->setVariable("TEMPDIR_NAME",_('Temporary Directory'));
$conttp->setVariable("TEMPDIR_VALUE",$txtTempdir);
$conttp->setVariable("PROTOCOL_NAME",_('Server protocol'));
...
// data settings
// ====================
$conttp->setVariable("DATA",_('Language'));
$conttp->setVariable("LOCALE",_('Language'));
...
$conttp->setVariable("ENCODING_NAME",_('Encoding'));
$conttp->setVariable("ENCODING_VALUE",$txtEncoding);
//
// database settings
//
$conttp->setVariable("DB",_('Database'));
$conttp->setVariable("SERVER_NAME",_('MySQL Server'));
$conttp->setVariable("SERVER_VALUE",$txtDBserver);
$conttp->setVariable("SERVER_PORT",_('MySQL Server Port'));
$conttp->setVariable("PORT_VALUE",$txtDBport);
$conttp->setVariable("DATABASE_NAME",_('Database name'));
$conttp->setVariable("DATABASE_VALUE",$txtDBname);
$conttp->setVariable("USERNAME_NAME",_('Database user'));
$conttp->setVariable("USERNAME_VALUE",$txtDBuser);
$conttp->setVariable("PASSWORD_NAME",_('Database password'));
$conttp->setVariable("PASSWORD_VALUE",$txtDBpass);
//
// security settings
//
$conttp->setVariable("SECURITY",_('Security'));
$conttp->setVariable("LOGOFFTIME_NAME",_('Session auto logoff time'));
...
//$conttp->setVariable("WSAUTH_NAME",_('Authentication type'));
...
// common settings
//
$conttp->setVariable("COMMON",_('Common'));
$conttp->setVariable("PAGELINES_NAME",_('Data lines per page'));
$conttp->setVariable("PAGELINES_VALUE",$txtLines);
$conttp->parse("SeldisableSelection");
$conttp->setVariable("SELDISABLE_NAME",_('Selection method'));
...
$conttp->setVariable("LANG_REQUIRED", _('required'));
