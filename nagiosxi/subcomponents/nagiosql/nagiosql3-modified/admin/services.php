...
if (($chkModus == "checkform") || ($chkModus == "filter")) {
  ...
  if ($chkSelOrderBy == _('All configs')) {
...
if ($chkServiceCommand != "") {
  for ($i=1;$i<=8;$i++) {
  	// XI MOD 02-10-2010 EG Allow empty $ARGx$ macros
...
// XI MOD 01-25-2010 EG Removed space replacement in config name
...
//    $intInsert = $myDataClass->dataInsert($strSQL,$intInsertId);
  if (($chkTfName != "") && ($chkTfServiceDescription != "")) {
    ...
    if ($intInsert == 1) {
      ...
      if ($chkModus  == "insert")   $myDataClass->writeLog(_('New service inserted:')." ".$chkTfName);
      if ($chkModus  == "modify")   $myDataClass->writeLog(_('Service modified:')." ".$chkTfName);
      //
      ...
      if (($chkModus == "modify") && ($chkOldConfig != $chkTfName)) {
        ...
        if ($intServiceCount == 0) {
          ...
          if ($intReturn == 0) {
            ...
            $myDataClass->writeLog(_('Service file deleted:')." ".$chkOldConfig.".cfg");
          } else {
            $strMessage .=  _('Errors while deleting the old configuration file - please check!:')."<br>".$myConfigClass->strDBMessage."<br>";
          }
        }
      }
      ...
      if (($chkModus == "modify") && ($chkActive == 0)) {
        ...
        if ($intServiceCount == 0) {
          ...
          if ($intReturn == 0) {
            $strMessage .=  _('The assigned, no longer used configuration files were deleted successfully!')."<br>";
            $myDataClass->writeLog(_('Service file deleted:')." ".$chkTfName.".cfg");
          } else {
            $strMessage .=  _('Errors while deleting the old configuration file - please check!:')."<br>".$myConfigClass->strDBMessage."<br>";
          }
        }
  ...
  if ($intError == 0) {
    $strMessage .= _('Configuration file successfully written!')."<br>";
    $intReturn = 0;
  } else {
  ...
  if ($booReturn == false) {
    $myDataClass->strDBMessage .= _('Error while selecting data from database:')."<br>".$myDBClass->strDBError."<br>";
    $intReturn = 1;
  }
  $chkModus      = "add";
  ...
  if ($arrActiveData['counter'] == 0) {
    $intReturn = $myConfigClass->moveFile("service",$chkOldConfig.".cfg");
    if ($intReturn == 0) {
      $strMessage .=  _('The assigned, no longer used configuration files were deleted successfully!');
      $myDataClass->writeLog(_('Service file deleted:')." ".$chkOldConfig.".cfg");
    } else {
      $strMessage .=  _('Errors while deleting the old configuration file - please check!:')."<br>".$myConfigClass->strDBMessage;
    }
...
// Content einbinden
// =================
$conttp->setVariable("TITLE",_('Define services (services.cfg)'));
$conttp->parse("header");
$conttp->show("header");
//
// Eingabeformular
...
if ($chkModus == "add") {
  ...
  if (($intReturn1 != 0) && ($intReturn2 != 0)) $strDBWarning .= _('Attention, no hosts or hostgroups defined!')."<br>";
  ...
  if ($intReturn != 0) $strDBWarning .= _('Attention, no check commands defined!')."<br>";
  ...
  if (($intReturn1 != 0) && ($intReturn1 != 0)) $strDBWarning .= _('Attention, no contact or contact groups defined!')."<br>";
...
if ($chkModus == "display") {
  ...
  $mastertp->setVariable("FIELD_1",_('Config name'));
  $mastertp->setVariable("FIELD_2",_('Service name'));
  ...
  $mastertp->setVariable("DAT_SEARCH",$_SESSION['serviceSearch']);
   // Configauswahl
  $mastertp->setVariable("DAT_CONFIGNAME",_('All configs'));
  ...
  if ($booReturn == false) {
    ...
    for ($i=0;$i<$intDataCount;$i++) {
      ...
      $mastertp->setVariable("DATA_FILE","<span class=\"dbmessage\">"._('out-of-date')."</span>");
      $mastertp->setVariable("LINE_ID",$arrDataLines[$i]['id']);
      $mastertp->setVariable("CELLCLASS_L",$strClassL);
      $mastertp->setVariable("CELLCLASS_M",$strClassM);
      $mastertp->setVariable("CHB_CLASS",$strChbClass);
      $mastertp->setVariable("IMAGE_PATH",$SETS['path']['root']."images/");
      if ($intOlder == 0) $mastertp->setVariable("DATA_FILE",_('up-to-date'));
      if ($intDate  == 1) $mastertp->setVariable("DATA_FILE","<span class=\"dbmessage\">"._('missed')."</span>");
