...
$strConfigfile  = $strBaseDir."/nagios.cfg";
$strTempfile  = "/nagiosql_config_temp.dat";
$strLocalBackup = $strBaseDir."/nagios.cfg_old_".$strOldDate;
//
// Daten verarbeiten
// =================
if ($chkNagiosConf != "") {
  ...
  if ($intMethod == 1) {
    if (file_exists($strConfigfile) && (is_writable($strConfigfile))) {
      $myConfigClass->moveFile("nagiosbasic","nagios.cfg");
      ...
      fclose($resFile);
      $strMessage .= "<span style=\"color:green\">"._('Configuration file successfully written!')."</span>";
      $myDataClass->writeLog(_('Configuration successfully written:')." ".$strConfigfile);
    } else {
      $strMessage = _('Cannot open/overwrite the configuration file (check the permissions)!');
      $myDataClass->writeLog(_('Configuration write failed:')." ".$strConfigfile);
    }
  } else if ($intMethod == 2) {
    ...
    $intReturn = $myConfigClass->configCopy("nagios.cfg",$strTempfile,"basic",1,1);
    if ($intReturn == 0) {
      $strMessage .= "<span style=\"color:green\">"._('Configuration file successfully written!')."</span>";
      $myDataClass->writeLog(_('Configuration successfully written:')." ".$strConfigfile);
      unlink($SETS['path']['tempdir']."/nagiosql_config_temp.dat");
    } else {
      $strMessage = _('Cannot open/overwrite the configuration file (check the permissions on FTP remote system)!');
      $myDataClass->writeLog(_('Configuration write failed (FTP remote):')." ".$strConfigfile);
    }
  }
}
//
...
// Content einbinden
// =================
$conttp->setVariable("TITLE",_('Nagios main configuration file'));
$conttp->parse("header");
$conttp->show("header");
//
// Eingabeformular
...
if ($intMethod == 1) {
  if (file_exists($strConfigfile) && is_readable($strConfigfile)) {
    ...
    $strMessage = _('Cannot open the data file (check the permissions)!');
  }
} else if ($intMethod == 2) {
  $intReturn = $myConfigClass->configCopy("nagios.cfg","nagiosql_config_temp.dat","basic",0);
  if ($intReturn == 0) {
