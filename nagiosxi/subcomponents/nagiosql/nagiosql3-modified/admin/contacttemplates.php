...
// =================
if ($intVersion == 3) {
	if ($chbHOnull3 == "") {$strHO = substr($chbHOd3.$chbHOu3.$chbHOr3.$chbHOf3.$chbHOs3.$chbHOn3,0,-1);} 			else {$strHO = "null";}
	if ($chbSOnull3 == "") {$strSO = substr($chbSOw3.$chbSOu3.$chbSOc3.$chbSOr3.$chbSOf3.$chbSOs3.$chbSOn3,0,-1);} 	else {$strSO = "null";}
} else {
...
if (($chkModus == "insert") || ($chkModus == "modify")) {
	...
	$strSQLx = "tbl_contacttemplate SET template_name='$chkTfName', alias='$chkTfFriendly', contactgroups=$intContactGroups,
	...
	if ($chkTfName != "") {	
		...
		if ($intInsert == 1) {
			...
			if ($chkModus  == "insert") 	$myDataClass->writeLog(_('New contact inserted:')." ".$chkTfName);
			if ($chkModus  == "modify") 	$myDataClass->writeLog(_('Contact modified:')." ".$chkTfName);
			//
			// Relationen eintragen/updaten
			// ============================
			if ($chkModus == "insert") {
				if ($intContactGroups  == 1) $myDataClass->dataInsertRelation("tbl_lnkContacttemplateToContactgroup",$chkDataId,$chkSelContactGroup);
				if ($intHostCommand    == 1) $myDataClass->dataInsertRelation("tbl_lnkContacttemplateToCommandHost",$chkDataId,$chkSelHostCommand);
				if ($intServiceCommand == 1) $myDataClass->dataInsertRelation("tbl_lnkContacttemplateToCommandService",$chkDataId,$chkSelServiceCommand);
			} else if ($chkModus == "modify") {		
				if ($intContactGroups == 1) {
					$myDataClass->dataUpdateRelation("tbl_lnkContacttemplateToContactgroup",$chkDataId,$chkSelContactGroup);
				} else {
					$myDataClass->dataDeleteRelation("tbl_lnkContacttemplateToContactgroup",$chkDataId);
				}
				if ($intHostCommand == 1) {
					$myDataClass->dataUpdateRelation("tbl_lnkContacttemplateToCommandHost",$chkDataId,$chkSelHostCommand);
				} else {
					$myDataClass->dataDeleteRelation("tbl_lnkContacttemplateToCommandHost",$chkDataId);			
				}
				if ($intServiceCommand == 1) {
					$myDataClass->dataUpdateRelation("tbl_lnkContacttemplateToCommandService",$chkDataId,$chkSelServiceCommand);
				} else {
					$myDataClass->dataDeleteRelation("tbl_lnkContacttemplateToCommandService",$chkDataId);			
				}
			}
			//
	...
	$intReturn = $myConfigClass->createConfig("tbl_contacttemplate",0);
	$chkModus  = "display";
}  else if (($chkModus == "checkform") && ($chkSelModify == "info")) {
	// Konfigurationsdatei schreiben
	$intReturn  = $myDataClass->infoRelation("tbl_contacttemplate",$chkListId,"template_name");
	$strMessage	= $myDataClass->strDBMessage;
	$intReturn  = 0;
	$chkModus   = "display";
}  else if (($chkModus == "checkform") && ($chkSelModify == "delete")) {
	// Gewählte Datensätze löschen
	$intReturn = $myDataClass->dataDeleteFull("tbl_contacttemplate",$chkListId);
	$strMessage .= $myDataClass->strDBMessage;	
	$chkModus  = "display";
} else if (($chkModus == "checkform") && ($chkSelModify == "copy")) {
	// Gewählte Datensätze kopieren
	$intReturn = $myDataClass->dataCopyEasy("tbl_contacttemplate","template_name",$chkListId);
	$chkModus  = "display";
} else if (($chkModus == "checkform") && ($chkSelModify == "modify")) {
	// Daten des gewählten Datensatzes holen
	$booReturn = $myDBClass->getSingleDataset("SELECT * FROM tbl_contacttemplate WHERE id=".$chkListId,$arrModifyData);
	if ($booReturn == false) $strMessage .= _('Error while selecting data from database:')."<br>".$myDBClass->strDBError."<br>";
	$chkModus      = "add";
}
...
// ======================================
$myConfigClass->lastModified("tbl_contacttemplate",$strLastModified,$strFileDate,$strOld);
//
// Menu aufbauen
// =============
...
// Content einbinden
// =================
$conttp->setVariable("TITLE",_("Define contact templates (contacttemplates.cfg)"));
$conttp->parse("header");
$conttp->show("header");
//
// Eingabeformular
...
if ($chkModus == "add") {
	...
	$strSQL 	= "SELECT id,template_name FROM tbl_contacttemplate WHERE config_id=".$_SESSION['domain']." $strWhere ORDER BY template_name";
	...
	$strSQL 	= "SELECT id, name FROM tbl_contact WHERE name <> '' AND config_id=".$_SESSION['domain']." ORDER BY name";
	...
	$intReturn = $myVisClass->parseSelect('tbl_timeperiod','timeperiod_name','DAT_TIMEPERIOD','timeperiodgroup1',$conttp,$chkListId,'',$intFieldId,1,0,5);
	...
	$intReturn = $myVisClass->parseSelect('tbl_timeperiod','timeperiod_name','DAT_TIMEPERIOD','timeperiodgroup2',$conttp,$chkListId,'',$intFieldId,1,0,5);
	if ($intReturn != 0) $strDBWarning .= _('Attention, no time periods defined!')."<br>";
	...
	$intReturn = $myVisClass->parseSelect('tbl_command','command_name','DAT_COMMAND1','commandgroup1',$conttp,$chkListId,'tbl_lnkContacttemplateToCommandHost',$intFieldId,2,0,2);
	...
	$intReturn = $myVisClass->parseSelect('tbl_command','command_name','DAT_COMMAND2','commandgroup2',$conttp,$chkListId,'tbl_lnkContacttemplateToCommandService',$intFieldId,2,0,2);
	if ($intReturn != 0) $strDBWarning .= _('Attention, no commands defined!')."<br>";
	// Kontaktgruppenfeld setzen
	if (isset($arrModifyData['contactgroups'])) {$intFieldId = $arrModifyData['contactgroups'];} else {$intFieldId = 0;}
	$intReturn = $myVisClass->parseSelect('tbl_contactgroup','contactgroup_name','DAT_CONTACTGROUP','contactgroup',$conttp,$chkListId,'tbl_lnkContacttemplateToContactgroup',$intFieldId,2);
	...
	if ($strDBWarning != "") $conttp->setVariable("WARNING",$strDBWarning._('Saving not possible!'));
	...
	if (isset($arrModifyData) && ($chkSelModify == "modify")) {
		...
		if ($myDataClass->infoRelation("tbl_contacttemplate",$arrModifyData['id'],"template_name") != 0) {
			$conttp->setVariable("ACT_DISABLED","disabled");
			$conttp->setVariable("ACT_CHECKED","checked");
			$conttp->setVariable("ACTIVE","1");
			$strInfo = "<br><span class=\"dbmessage\">"._('Entry cannot be activated because it is used by another configuration').":</span><br><span class=\"greenmessage\">".$myDataClass->strDBMessage."</span>";
...
if ($chkModus == "display") {
	...
	$mastertp->setVariable("FIELD_1",_('Contact name'));
	$mastertp->setVariable("FIELD_2",_('Description'));
	$mastertp->setVariable("LIMIT",$chkLimit);
	$mastertp->setVariable("ACTION_MODIFY",$_SERVER['PHP_SELF']);
	$mastertp->setVariable("TABLE_NAME","tbl_contacttemplate");
	// Anzahl Datensätze holen
	$strSQL    = "SELECT count(*) AS number FROM tbl_contacttemplate WHERE config_id=$chkDomainId";
	...
	// Datensätze holen
	$strSQL    = "SELECT id, template_name, alias, active FROM tbl_contacttemplate WHERE config_id=$chkDomainId
	...
	if ($booReturn == false) {
		...
		for ($i=0;$i<$intDataCount;$i++) {	
			...
			if ($arrDataLines[$i]['active'] == 0) {$strActive = _('No');} else {$strActive = _('Yes');}	
			// Datenfelder setzen
			foreach($arrDescription AS $elem) {
				$mastertp->setVariable($elem['name'],$elem['string']);
			}
		...
		$mastertp->setVariable("DATA_FIELD_1",_('No data'));
...
$mastertp->setVariable("LAST_MODIFIED",_('Last database update:')." <b>".$strLastModified."</b>");
$mastertp->setVariable("FILEDATE",_('Last change of the configuration file:')." <b>".$strFileDate."</b>");
