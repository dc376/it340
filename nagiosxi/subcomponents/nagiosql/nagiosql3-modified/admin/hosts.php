...
//    (($intSelContacts != 0) || ($intSelContactGroups != 0))) {
  if ($chkTfName != "") {
    ...
    if ($intInsert == 1) {
      ...
      if ($chkModus  == "insert")   $myDataClass->writeLog(_('New host inserted:')." ".$chkTfName);
      if ($chkModus  == "modify")   $myDataClass->writeLog(_('Host modified:')." ".$chkTfName);
      //
      ...
      if (($chkModus == "modify") && ($chkOldHost != $chkTfName)) {
        ...
        if ($intReturn == 0) {
          ...
          $myDataClass->writeLog(_('Host file deleted:')." ".$chkOldHost.".cfg");
        } else {
      ...
      if (($chkModus == "modify") && ($chkActive == 0)) {
        ...
        if ($intReturn == 0) {
          $strMessage .=  _('The assigned, no longer used configuration files were deleted successfully!');
          $myDataClass->writeLog(_('Host file deleted:')." ".$chkTfName.".cfg");
        } else {
          $strMessage .=  _('Errors while deleting the old configuration file - please check!:')."<br>".$myConfigClass->strDBMessage;
        }
      }
...
// Content einbinden
// =================
$conttp->setVariable("TITLE",_('Define hosts (hosts.cfg)'));
$conttp->parse("header");
$conttp->show("header");
//
// Eingabeformular
...
if ($chkModus == "add") {
  ...
  if (($intReturn != 0) && ($intReturn2 != 0)) $strDBWarning .= _('Attention, no contact groups defined!')."<br>";
...
if ($chkModus == "display") {
  ...
  $mastertp->setVariable("FIELD_1",_('Host name'));
