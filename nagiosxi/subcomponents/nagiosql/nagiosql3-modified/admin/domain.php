...
if (($chkModus == "modify" || $chkModus == "insert") AND $chkInsMethod == 1) {
  ...
  if ($isanerror == 1) {
    ...
    $strErrMessage .= "<h2>"._("Warning, at least one error occured, please check!")."</h2>";
...
if (($chkModus == "insert") || ($chkModus == "modify")) {
  ...
  if (($chkInsDomain != "") && ($chkInsAlias != "") && ($chkInsServer != "")) {
    ...
    if ($chkModus  == "insert")   $myDataClass->writeLog(_('New Domain inserted:')." ".$chkInsDomain);
    if ($chkModus  == "modify")   $myDataClass->writeLog(_('Domain modified:')." ".$chkInsDomain);
  } else {
  ...
  if ($chkHidDomain != "localhost") {
    ...
    $myDataClass->strDBMessage = _("Localhost can't be deleted");
  }
  $chkModus  = "display";
...
// Content einbinden
// =================
$conttp->setVariable("TITLE",_('Domain administration'));
...
if ($chkModus == "add") {
  ...
  $conttp->setVariable("LANG_ACCESSDESCRIPTION",_('In order for a user to get access, he needs to have a key for each key hole defined here'));
  ...
  $conttp->setVariable("FILL_ILLEGALCHARS",_('The following field contains not permitted characters:'));
...
if ($chkModus == "display") {
  ...
  $mastertp->setVariable("FIELD_1",_('Domain'));
