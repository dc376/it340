...
function DirToArray($sPath, $include, $exclude, &$output,&$errMessage) {
  ...
  if( $handle === false ) {
    $errMessage .= _('Could not open directory')." ".$sPath."<br>";
  } else {
...
if ($chkSelFilename[0] != "") {
  ...
  foreach($chkSelFilename AS $elem) {
    ...
    if ($intCheck == 0) {
      $myDataClass->writeLog(_("File deleted").": ".trim($elem));
      $strMessage .= $elem." "._("successfully deleted")."!<br>";
    } else {
      $strMessage .= $elem." "._("could not be deleted (check the permissions)")."!<br>";
...
// Content einbinden
// =================
$conttp->setVariable("TITLE",_("Delete backup files"));
$conttp->parse("header");
$conttp->show("header");
$conttp->setVariable("LANG_SEARCH_STRING",_('Filter string'));
$conttp->setVariable("LANG_SEARCH",_('Search'));
$conttp->setVariable("LANG_DELETE",_('Delete'));
$conttp->setVariable("DAT_SEARCH",$chkSearch);
$conttp->setVariable("BACKUPFILE",_("Backup file"));
$conttp->setVariable("MUST_DATA","* "._('required'));
$conttp->setVariable("MAKE",_("Delete"));
$conttp->setVariable("ABORT",_("Abort"));
$conttp->setVariable("CTRL_INFO",_("Hold CTRL to select<br>more than one entry"));
