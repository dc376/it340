...
// Content einbinden
// =================
$conttp->setVariable("TITLE",_("Define contacts (contacts.cfg)"));
$conttp->parse("header");
$conttp->show("header");
//
// Eingabeformular
...
if ($chkModus == "add") {
  ...
  if (isset($arrModifyData) && ($chkSelModify == "modify")) {
    ...
    if ($myDataClass->checkMustdata("tbl_contact",$arrModifyData['id'],$arrInfo) != 0) {
      $conttp->setVariable("ACT_DISABLED","disabled");
      $conttp->setVariable("ACTIVE","1");
      $conttp->setVariable("CHECK_MUST_DATA","<span class=\"dbmessage\">"._('Entry cannot be deactivated because it is used by another configuration')."</span>");
    }
