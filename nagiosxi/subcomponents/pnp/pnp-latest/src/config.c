...
//
// $Id: config.c 648 2009-08-11 11:53:59Z hendrikb $
	...
	/* ok - parsing the config file line by line */
		...
		while (feof(fh_config_file) == 0) {
			...
			/* log_type directive */
				...
				printf(
						"A warning occured while reading your config on line %d. Message was: \"%s\"\n",
...
int prepare_vars() {
	...
	if (macro_x[CONFIG_OPT_SCANDIR] != NULL) {
		...
		printf(
				"WARNING - Using a hardcoded default perfdata spooldir - '%s'\n",
				directory);
	}
