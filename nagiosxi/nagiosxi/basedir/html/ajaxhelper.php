...
function ah_set_user_meta()
{
    ...
    $allowed = array('view_rotation_speed', 'view_rotation_enabled', 'show_login_alert_screen', 'wizard_import_only', 'ccm_stop_import_info_popup', 'day_45', 'day_30', 'day_15', 'day_5', 'day_1', 'exp_10', 'exp_30', 'exp_45', 'exp_60', 'exp_no_show', 'downtime_page_refresh');
...
function ah_get_dashboard_select_menu_html()
{
    ...
    if (count($dashboards) == 0) {
        ...
        $dashboards = get_dashboards(0); // Grab new dashboard
    }

    foreach ($dashboards as $d) {
        if ($d["id"] == SCREEN_DASHBOARD_ID) continue;
...
function ah_get_apply_config_changes()
{
    ...
    $contents = file_get_contents($cfg['script_dir'] . '/nagiosql.export.monitoring');
    $start = strpos($contents, "<div id='applyConfigOutput' class='success floatLeft'>");
...
function ah_dbg_tail_log() {
    ...
    $handle = fopen('/tmp/xidebug.log', 'r');
...
function ah_dbg_truncate_log() {
    $cmd = "echo \"\" > /tmp/xidebug.log";
    @exec($cmd);
}

function ah_dbg_truncate_backtrace_log() {
    $cmd = "echo \"\" > /tmp/xidebug.log.backtrace";
    @exec($cmd);
}
