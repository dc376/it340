...
function route_request()
{
    ...
    // disable so old rr link will not work if $cfg['secure_response_url']=1
    if (isset($cfg['secure_response_url']) && $cfg['secure_response_url'] == 1) {
        echo _("Rapid Response URL's have been secured.  this link is no longer valid.");
        exit();
    }
...
function process_command()
{
    ...
    $comment = grab_request_var("comment", "Problem is acknowledged");
    $duration = intval(grab_request_var("duration", "")) * 60;

    //print_r($request);

    if ($comment == "")
        $comment = "Problem is acknowledged";
    if ($service != "")
    ...
    switch ($action) {
        ...
        case "disablenotifications":
            ...
            //echo "CMD: $cmdstr<BR>";
            $output = _("Notifications Disabled");
            ...
            if ($service != "")
                ...
                $obj = "host '" . $host . "'";
            send_to_audit_log(_("User disabled notifications for ") . $obj . _(" via Rapid Response"), AUDITLOGTYPE_CHANGE);
            break;
        ...
        case "acknowledge":
            ...
            if ($service != "")
                ...
                $obj = "host '" . $host . "'";
            send_to_audit_log(_("User acknowledged problem state for ") . $obj . _(" via Rapid Response"), AUDITLOGTYPE_CHANGE);
        ...
        case "scheduledowntime":
            ...
            if ($service != "")
                ...
                $obj = "host '" . $host . "'";
            send_to_audit_log(_("User scheduled downtime for ") . $obj . _(" via Rapid Response"), AUDITLOGTYPE_CHANGE);
    ...
    <p><b><?php echo _("What next?"); ?></b></p>
    <a href="<?php echo $detail_url; ?>"><?php echo _("View") . " ";
        echo ($service == "") ? _("Host") : _("Service");
        echo " " . _("Details"); ?></a><br>
    <a href="<?php echo get_option("url"); ?>"><?php echo _("Go to the Nagios dashboard"); ?></a><br>
...
function show_options($args)
{
    ...
    $page_title = _("Nagios Rapid Response");
        ...
        <div class="servicestatusdetailheader">
            ...
            <div class="servicetitle">
                ...
                <div class="hostname"><a
                        href="<?php echo get_host_status_detail_link($host); ?>"><?php echo htmlentities($host); ?></a>
                </div>
            </div>
        </div>
        <br clear="all">
    ...
    <form method="get" action="<?php echo htmlentities($_SERVER["REQUEST_URI"]); ?>"">
		...
		<table border="0">
            ...
            <tr>
                ...
                <td>
                    <select name="action" id="actionList">
                        <option value="visit"><?php echo _("View ");
                            echo ($service == "") ? _("Host ") : _("Service ");
                            echo _("Detail"); ?></option>
                        <?php
                        ...
                        if (($service == '' && is_authorized_for_host_command($_SESSION['user_id'], $host))
                            ...
                            if ($problem_acknowledged == 0) {
                                ?>
                                <option value="acknowledge"><?php echo _("Acknowledge"); ?></option>
                            <?php }
                            if ($scheduled_downtime_depth == 0) { ?>
                                <option value="scheduledowntime"><?php echo _("Schedule Downtime"); ?></option>
                            <?php }
            ...
            <tr id="commentfield">
                ...
                <td>
                    <input type="text" name="comment" size="30" value="<?php echo _("Problem is acknowledged"); ?>"><br>
                </td>
            </tr>
            ...
            <tr id="durationfield">
                ...
                <td>
                    <input type="text" name="duration" size="2" value="60"> <?php echo _("Minutes"); ?><br/>
                </td>
            </tr>
            ...
            <tr>
                <td></td>
                <td><input type="submit" value="Go" name="btnSubmit"></td>
            </tr>
