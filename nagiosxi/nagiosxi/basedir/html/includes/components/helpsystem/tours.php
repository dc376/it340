...
//  
// $Id: tours.php 903 2015-07-30 21:15:05Z swilkerson $
...
function child_tour($child_page){
    
    switch($child_page){
            case 1000: // /nagiosxi/config/monitoringwizard.php
                $steps[] = array(
                'backdrop' => true,
                'orphan' => true,
                'element' => "",
                'title' => _('Nagios XI Configuration Wizards'),
                'content' => _('Configuration wizards make it easy to setup the monitoring configuration for novice or advanced Nagios users.')
                );
                $steps[] = array(
                    'backdrop' => false,
                    'element' => ".fa-suitcase",
                    'placement' => "right",
                    'title' => _('Filtering'),
                    'content' => _('With so many wizards to choose from, we have made it easier to find the wizard you are looking for by using filters.')
                    );            
                     break;
    ...
    $steps = build_tour_steps($steps);
    $tour = build_tour_js($child_page, $steps);
