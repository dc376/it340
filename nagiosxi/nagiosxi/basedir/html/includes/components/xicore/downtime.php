...
function show_downtime()
{
    ...
    $refresh = get_user_meta(0, 'downtime_page_refresh', 30);
        ...
        success: function(dt) {
            ...
            if (Object.keys(dt).length > 0) {
                $.each(dt, function(k, d) {
                    ...
                    if (d.is_in_effect) { d.is_in_effect = "<?php echo _('Yes'); ?>"; } else { d.is_in_effect = "<?php echo _('No'); ?>" }
    ...
    $('.do-for-selected').change(function() {
        if ($(this).val() == 'remove') {
            if (SEL_IDS.length > 0) {
                ...
                $('#confirm-rd').css('max-width', rd_width).center().css('top', 160).show();
...
<div class="well report-options">
    <div class="fl" style="height: 29px; margin-right: 30px;">
        <h1 style="padding: 0; line-height: 29px; display: inline-block;"><?php echo _('Scheduled Downtime'); ?></h1>
        <a class="tt-bind refresh" style="vertical-align: top; margin: 7px 0 0 5px; height: 16px; width: 16px; color: #222; display: inline-block;" data-placement="right" title="<?php echo _('Refresh'); ?>"><img src="<?php echo theme_image('arrow_refresh.png'); ?>"></a>
        <a class="tt-bind" id="open-dtform" style="vertical-align: top; margin: 7px 0 0 5px; height: 16px; width: 16px; color: #222; display: inline-block;" data-placement="right" title="<?php echo _('Edit page settings'); ?>"><img src="<?php echo theme_image('cog.png'); ?>"></a>
    ...
    <div class="input-group fl" style="width: 200px;">
        <label class="input-group-addon"><?php echo _('Filter by'); ?></label>
        <select class="filters hide" name="filters" multiple>
            <option value="host">Host</option>
            <option value="service">Service</option>
            <option value="ineffect">In effect</option>
            <option value="notineffect">Not in effect</option>
        </select>
    </div>
    <div class="fr">
    ...
    <div class="fl">
        ...
        <div class="btn-group fl" style="margin-right: 20px;">
            ...
            <ul class="dropdown-menu">
                <li><a href="?cmd=schedule&type=host"><?php echo _('For Host(s)'); ?></a></li>
                <li><a href="?cmd=schedule&type=service"><?php echo _('For Service(s)'); ?></a></li>
            </ul>
        </div>
        <?php } ?>
    ...
    <table class="table table-condensed table-hover table-downtime table-bordered table-striped table-no-margin" style="margin-top: 10px;">
        <thead>
            <tr>
                ...
                <th class="time-fw"><?php echo _('Entry Time'); ?></th>
                <th><?php echo _('Author'); ?></th>
                <th><?php echo _('Comment'); ?></th>
                <th class="time-fw"><?php echo _('Start Time'); ?></th>
                <th class="time-fw"><?php echo _('End Time'); ?></th>
...
function show_add_downtime($error = '')
{
    ...
    $get = '?username='.urlencode($_SESSION['username']).'&ticket='.get_user_attr(0, 'backend_ticket').'&query=downtimelist&details=true';
    $dt_url = get_base_url()."includes/components/nagioscore/ui/statusjson.php".$get;
...
<form action="?cmd=submit" method="post">
    ...
    <table class="table table-auto-width table-no-border" style="width: 50%; min-width: 800px;">
                                        ...
                                        if (count($services) > 0) {
                                            ...
                                            <tr>
                                                <td>
                                                    ...
                                                    <input type="hidden" name="services[<?php echo $host; ?>][]" value="<?php echo htmlentities($service); ?>">
        ...
        <tr>
            ...
            <td><input type="text" class="form-control" name="com_data" style="width: 400px;" value="<?php echo htmlentities($com_data); ?>"></td>
        </tr>
        <tr>
            ...
            <td>
                <?php
                $ch = curl_init();
                curl_setopt($ch, CURLOPT_URL, $dt_url);
                ...
                <select class="form-control" name="trigger" id="trigger">
                    ...
                    foreach ($objs->data->downtimelist as $obj) { 
        ...
        <tr>
            ...
            <td>
                <input class="form-control datetimepicker req" style="width: 150px;" type="text" id='startdateBox' name="start_time" value="<?php echo htmlentities($start_time); ?>" size="16">
            </td>
        </tr>
        <tr>
            <td><?php echo _("End Time"); ?> <i class="fa fa-asterisk fa-req tt-bind" title="<?php echo _('Required'); ?>"></i></td>
            <td>
                <input class="form-control datetimepicker req" style="width: 150px;" type="text" id='enddateBox' name="end_time" value="<?php echo htmlentities($end_time); ?>" size="16">
            </td>
        </tr>
...
function do_submit_downtime()
{
    ...
    $cmd_mod = 2;
    $cmd_id = NAGIOSCORE_CMD_SCHEDULE_HOST_DOWNTIME;
    if ($type == 'service') {
        $cmd_id = NAGIOSCORE_CMD_SCHEDULE_SVC_DOWNTIME;
    }
    ...
    if (nstrtotime($start_time) > nstrtotime($end_time)) {
    ...
    if ($type == 'host') {
        ...
        foreach ($hosts as $h) {
            $args['host'] = $h;
            $x = send_scheduled_downtime($args);
            if (!$x) { $errors++; }

            // Check if we should schedule service downtimes too
            if ($serviceoptions != 0) {
                $args['cmd_typ'] = NAGIOSCORE_CMD_SCHEDULE_HOST_SVC_DOWNTIME;
                $x = send_scheduled_downtime($args);
                if (!$x) { $errors++; }
            }
        }
...
function send_scheduled_downtime($args) {

    // Submit to backend for processing
    $get = '?username='.urlencode($_SESSION['username']).'&ticket='.get_user_attr(0, 'backend_ticket');
    $url = get_base_url()."includes/components/nagioscore/ui/cmd.php".$get;
    ...
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_USERAGENT, 'Nagios XI Curl Request');
    curl_setopt($ch, CURLOPT_POST, 1);
    curl_setopt($ch, CURLOPT_POSTFIELDS, $args);
    $output = curl_exec($ch);
    curl_close($ch);

    $success = strpos($output, "Your command request was successfully submitted to Nagios for processing.");
    if ($success === false) {
        return false;
    }
    return true;
...
function get_downtimes($total=false) {
    ...
    if ($total) {
        $recordcount = 0;
        if ($xml) {
            $recordcount = intval($xml->recordcount);
        } else {
            return false;
        }
        return $recordcount;
    ...
    if ($xml) {
        foreach ($xml->scheduleddowntime as $x) {
            $start_ts = convert_datetime_to_timestamp($x->scheduled_start_time);
            $end_ts = convert_datetime_to_timestamp($x->scheduled_end_time);
            $duration = ($end_ts - $start_ts) / 1000;
            $downtimes["$x->internal_id"] = array(
                "downtime_id" =>            "$x->internal_id",
                "type" =>                   "$x->downtime_type",
                "host_name" =>              "$x->host_name",
                "entry_time" =>             convert_datetime_to_timestamp($x->entry_time),
                "start_time" =>             $start_ts,
                "end_time" =>               $end_ts,
                "author" =>                 "$x->author_name",
                "comment" =>                "$x->comment_data",
                "fixed" =>                  intval($x->fixed),
                "duration" =>               "$duration",
                "triggered_by" =>           intval($x->triggered_by),
                "is_in_effect" =>           intval($x->was_started),
                );
            if ($x->service_description != "") {
                $downtimes["$x->internal_id"]["service_description"] = "$x->service_description";
            }
        }
    } else {
        return false;
