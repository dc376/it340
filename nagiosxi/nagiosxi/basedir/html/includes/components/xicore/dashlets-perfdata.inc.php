...
// Display a performance graph dsahlet
/**
 * @param string $mode
 * @param string $id
 * @param null   $args
...
function xicore_dashlet_perfdata_get_highcharts($id, $args, $mode)
{
    ...
    // Next page link
    $link = base64_encode(get_base_url() . 'perfgraphs/index.php?search=&host=' . urlencode($hostname) . '&view=' . $view . '&startdate=' . $startdate . '&enddate=' . $enddate . '&mode=' . $link_mode);
