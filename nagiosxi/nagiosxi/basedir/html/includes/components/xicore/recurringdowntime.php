    ...
    if (count($schedules) > 0) {
        foreach ($schedules as $k => $schedule) {
            ...
            foreach ($lines as $k2 => $line) {
                ...
                $line_arr = array();
                foreach (explode("\t", trim($line)) as $k3 => $val) {
                    if ($val != "") {
                        $line_arr[] = trim($val);
                    }
                }
...
function recurringdowntime_get_service_cfg($host = false)
{
    ...
    foreach ($cfg as $sid => $schedule) {
        ...
        // Commented out because $request isn't even defined which means this never does anything
...
function recurringdowntime_add_downtime($edit = false) {
    ...
    if ($edit) {
        ...
        $selected_months = split(",", $formvars["months_of_year"]);
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $required = array(
            ...
            "duration" => _("Please enter the duration of this downtime event.")
        );
        ...
        if (!empty($request["duration"])) {
            if (!is_numeric($request["duration"])) {
                $errors[] = _("Please enter a valid duration time in seconds, e.g. 120.");
            }
        }
...
<?php } elseif ($form_mode == "service") { ?>
    ...
    <tr>
        ...
        <td>
            ...
            <div class="subtext"><?php echo _("Optional service associated with this schedule."); ?> <?php if (is_admin()) { ?><?php echo _("A wildcard can be used to specify multiple matches"); ?> (e.g. 'HTTP*').<?php } ?></div>
            <script type="text/javascript">
                $(document).ready(function () {
...
<tr>
    ...
    <td>
        ...
        <div class="subtext"><?php echo _("Duration of the scheduled downtime in minutes"); ?>.</div>
    </td>
</tr>
<tr>
    <td class="vt">
...
function recurringdowntime_show_downtime()
{
    ...
    do_page_start(array("page_title" => _("Recurring Scheduled Downtime")), true);
    ...
    ?>
    <h1><?php echo _("Recurring Scheduled Downtime"); ?></h1>
