...
function xicore_ajax_get_monitoring_proc_html($args = null)
{
    ...
    if ($xml == null) {
        ...
        if ($programrunning == 0) {
            ...
            // EXTERNAL COMMANDS
            // removed in preparation for 2014 (not really useful anyways -SW)
            /* 
			...
			if(($now-$cmdtime)>180)
				...
				$v=1;
			$output.='<tr><td><span class="sysstat_stat_subtitle">'._('External Commands').'</span></td><td>'.xicore_ajax_get_setting_status_html($v).'</td><td></td></tr>';
			*/
