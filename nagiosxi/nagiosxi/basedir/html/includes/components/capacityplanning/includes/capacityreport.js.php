...
function make_highcharts(data, host, service, track, uid, width, height) {
    ...
    $.each(series_data, function (index, series) {
        // Cache the fit line series for later...
        if (series.name == 'Fit') fit = series;
        // Change area series to line, and set arearange fill to white
        // when asked to hideoptions for PDF reports.
        if (hideoptions) {
    ...
    // Add arearange series for one and two adjusted sigmas around the fit.
    /*
    if (fit) {
        var fillColor = hideoptions ? 'white' : null;
        series_data.push(make_highcharts_fit_range(
            fit, '+/- 1 SE', 1 * data.adjusted_sigma, -1, 0.1, fillColor
        ));
        series_data.push(make_highcharts_fit_range(
            fit, '+/- 2 SE', 2 * data.adjusted_sigma, -2, 0.1, fillColor
        ));
    }
    */
    ...
    if (!hideoptions && start && step && stop) {
    ...
    var yAxis = {
        ...
        min: 0, // @todo: Revisit to support negative values.
        startOnTick: false,
    ...
    if (data.error) {
        chart.showLoading('<?php echo _("No data to display:"); ?> ' + data.error);
    }
}
...
function format_table_body( tbodyToModify ) {
  ...
  //console.log("TBody Height"+tbodyToModify.height());
}
