...
def linear_fit(dataset, extrap):    return polynomial_fit(dataset, extrap, 1, ' LinFit')
def quadratic_fit(dataset, extrap): return polynomial_fit(dataset, extrap, 2, ' QuadFit')
def cubic_fit(dataset, extrap):     return polynomial_fit(dataset, extrap, 3, ' CubeFit')
...
def holt_winters(dataset, extrap):
    ...
    I = np.asarray([ypoints[i] / (a_not + (i + 1) * b_not)
    ...
    for i in range(ylen):
        ...
        if debug_updates:
            print "i:", i, "y[i]:", ypoints[i], "S[i]:", S[i], "Atm1:", A_tm1, "Btm1:",B_tm1, "At:", A_t, "Bt:", B_t, "S[i+c]:", S[i+c], "F[i]:", F[i]

    if debug_updated_S:
        print 'S:', S
    for m in np.arange(extrapolate_cycles * c):
        if debug_updates and m == 0:
            print A_t, B_t, m+1, S[ylen + m], (A_t + B_t * (m + 1)) * S[ylen + m]
