            ...
            if (textbox.val().length > minimumTextValue || e.which === 46 || e.which === 8) {
                ...
                $.each(optionsAndOptGroups, function() {
                    if($(this).is('option')){
                        ...
                        var optionGroupClone = $(this).clone();
...
$(document).ready(function() {
    ...
    $('#subForm1').click(function() {
        ...
        if (objtype == 'service') {
            if ($('#tfServiceDescription').val().match(/[{}`~!$%^&*|'"<>?,()=\\]/)) {
        ...
        // Object names
        if (($('#tfName').val() != undefined) && $('#tfName').val().match(/[{}`~!$%^&*|'"<>?,()=\\]/)) {
        ...
        if (objtype == "hostescalation") {
            ...
            if (hosts_size == 0 && hostgroups_size == 0) {
                alert("Must define at least 1 host or hostgroup for a valid service escalation definition.");
                return false;
            } else if (services_size == 0) {
                alert("Must define at least 1 service for a valid service escalation definition.");
                return false;
            }
        }
        ...
        if (objtype == "servicedependency") {
            ...
            if (hosts_size == 0 && hostgroups_size == 0) {
                alert("Must define at least 1 host or hostgroup for a valid service dependency definition.");
                return false;
            } else if (services_size == 0) {
        ...
        // Verify that arguments don't have a ! in them or they break...
        $('.arg').each(function(k, v) {
            var cmd_arg = $(v).val();
            if (cmd_arg != "") {
                if (cmd_arg.indexOf('!') >= 0) {
                    alert("You can not use the ! in your command arguments.\n\nThe ! character is special, it denotes a separate arguement passed to a command.");
                    return false;
                }
            }
        });
    ...
    $('#run_command').click(function() {
        ...
        $('#command_output').html('<i class="fa fa-spinner fa-pulse fa-3x fa-fw"></i>').load(url);
    });
...
function transferMembers(inputDiv, outputDiv, postArray, afterLoad)
{
    ...
    for (i=0;i < ids.length; i++)
    {
        ...
        if (objtype == 'service' || objtype == 'serviceescalation' || objtype == 'hostescalation' || objtype == 'servicetemplate' || objtype == 'hostgroup') {
...
function insertDefinition(varName, varDef)
{
    ...
    string += '<td class="actions"><a class="xBox" href="javascript:void(0)" title="Remove" onclick="remove_row(\'tr'+unique+'\', \'\',\'\')"></a></td></tr>';
