...
function process_timeperiod_submission()
{
    ...
    if ($chkModus == "insert") {
        ...
        $myDataClass->writeLog(_('New time period inserted:')." ".$chkInsName);
    }

    if ($chkModus == "modify") {
        $myDataClass->writeLog(_('Time period modified:')." ".$chkInsName);
    }

    // Update relationships
    if ($chkModus == "insert") {
        if ($intSelExclude == 1) {
            $myDataClass->dataInsertRelation("tbl_lnkTimeperiodToTimeperiod", $chkDataId, $chkSelExclude);
        }
...
function process_command_submission()
{
    ...
    if ($intInsert == 1) {
        ...
        if ($chkModus == "insert") $myDataClass->writeLog(_('New command inserted:')." ".$chkInsName);
        if ($chkModus == "modify") $myDataClass->writeLog(_('Command modified:')." ".$chkInsName);
        $intReturn = 0;
    }
...
function process_escalation_submission()
{
    ...
    {
        if($chkModus == "insert") $myDataClass->writeLog(_('New service escalation inserted:')." ".$chkTfConfigName);
        if($chkModus == "modify") $myDataClass->writeLog(_('Service escalation modified:')." ".$chkTfConfigName);
      
      // Update Relations 
      // ============================
      if ($chkModus == "insert") 
      {
        if (!empty($chkSelHost)) $myDataClass->dataInsertRelation("tbl_lnk{$ucType}ToHost",$chkDataId,$chkSelHost,0,$chkSelHostExc);
        if (!empty($chkSelHostGroup)) $myDataClass->dataInsertRelation("tbl_lnk{$ucType}ToHostgroup",$chkDataId,$chkSelHostGroup,0,$chkSelHostGroupExc);
      ...
      {
        //update hosts 
        if (!empty($chkSelHost) && $intSelHost!=2) $myDataClass->dataUpdateRelation("tbl_lnk{$ucType}ToHost",$chkDataId,$chkSelHost,0,$chkSelHostExc);
        else  $myDataClass->dataDeleteRelation("tbl_lnk{$ucType}ToHost",$chkDataId);
        //update hostgroups 
        if (!empty($chkSelHostGroup) && $intSelHostGroup!=2) $myDataClass->dataUpdateRelation("tbl_lnk{$ucType}ToHostgroup",$chkDataId,$chkSelHostGroup,0,$chkSelHostGroupExc);
    ...
    if ($errors == 0) {
        ...
        // If we are saving for Nagios XI let's add the apply config needed
        if (ENVIRONMENT == "nagiosxi") {
            set_option("ccm_apply_config_needed", 1);
        }
    }
...
function process_dependency_submission()
{
    ...
    if($intInsert == 1) //there was a problem with the first entry 
        ...
        if ($chkModus == "insert") $myDataClass->writeLog(_('New service dependency inserted:')." ".$chkTfConfigName);
        if ($chkModus == "modify") $myDataClass->writeLog(_('Service dependency modified:')." ".$chkTfConfigName);
