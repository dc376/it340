...
function display_page($posted=false, $error='', $msg='') {
    ...
    // Grab the servers from the current server list
    ...
    <div>
                ...
                <tbody id="ldap_ad_servers">
                    ...
                    <?php
                        ...
                        <tr>
                            ...
                            <td><?php if ($server['conn_method'] == "ldap") { echo $server['ldap_host']; } else { echo $server['ad_domain_controllers']; } ?></td>
                            <td><?php echo ldap_ad_display_type($server['conn_method']); ?></td>
                            <td><?php echo strtoupper($server['security_level']); ?></td>
            ...
            <div <?php if (!$posted) { echo 'class="hide"'; } ?> id="manage-servers">
                ...
                <table style="margin-bottom: 20px;">
                    ...
                    <tr class="option ad-options ldap-options">
                        ...
                        <td valign="top">
                            <input type="text" size="40" name="base_dn" id="base_dn" value="<?php echo htmlentities($base_dn); ?>" placeholder="dc=nagios,dc=com" class="textfield form-control">
                    ...
                    <tr class="option ad-options <?php if ($conn_method == "ldap") { echo 'hide'; } ?>">
                        ...
                        <td>
                            <input type="text" size="25" name="account_suffix" id="account_suffix" value="<?php echo htmlentities($account_suffix); ?>" placeholder="@nagios.com" class="textfield form-control">
                    ...
                    <tr class="option ad-options <?php if ($conn_method == "ldap") { echo 'hide'; } ?>">
                        ...
                        <td valign="top">
                            <input type="text" size="80" name="domain_controllers" id="domain_controllers" value="<?php echo htmlentities($domain_controllers); ?>" placeholder="dc1.nagios.com,dc2.nagios.com" class="textfield form-control">
                    ...
                    <tr class="option ldap-options <?php if ($conn_method == "ad") { echo 'hide'; } ?>">
                        ...
                        <td>
                            <input type="text" name="ldap_host" id="ldap_host" value="<?php echo htmlentities($ldap_host); ?>" placeholder="ldap.nagios.com" class="textfield form-control" style="width: 300px;">
                    ...
                    <tr class="option ldap-options <?php if ($conn_method == "ad") { echo 'hide'; } ?>">
                        ...
                        <td>
                            <input type="text" name="ldap_port" id="ldap_port" value="<?php echo htmlentities($ldap_port); ?>" class="textfield form-control" style="width: 60px;">
                    ...
                    <tr class="option ad-options ldap-options">
                        ...
                        <td valign="top">
                            <select name="security_level" id="security_level" class="form-control">
                                ...
                                <option value="ssl" <?php echo ldap_ad_has_security($security_level, "ssl"); ?>>SSL</option>
                                <option value="tls" <?php echo ldap_ad_has_security($security_level, "tls"); ?>>TLS</option>
                            </select>
                            <div><?php echo _("The type of security (if any) to use for the connection to the server(s)."); ?></div>
                        </td>
                    </tr>
                </table>
                <p>
        ...
        <div style="width: 50%; min-width: 600px; float: left;">
            ...
            <p><?php echo _("For connecting over SSL/TLS using self-signed certificates you will need to add the certificate(s) of the domain controller(s) to the local certificate authority so they are trusted. If any certificate was signed by a host other than itself, that certificate authority/host certificate needs to be added."); ?></p>
    ...
    $(document).ready(function() {
        ...
        $("#add-ca-cert").click(function() {
            var content = '<div><h2><?php echo _("Add Certificate to Certificate Authority"); ?></h2><p><?php echo _("To add a certificate to the certificate authority, copy and paste the actual certificate between, and including, the begin/end certificate sections."); ?></p><div><label for="cahost"><?php echo _("Hostname"); ?></label></div><div><input name="host" class="form-control" id="cahost" type="text" style="width: 200px;" readonly> <i id="caload" class="fa fa-spinner fa-spin" style="font-size: 20px; vertical-align: middle; margin-left: 6px; display: none;"></i> <span id="caerror" style="display: none; margin-left: 6px; color: red;"></span></div><div style="margin-top: 5px;"><label><?php echo _("Certificate"); ?></label></div><div style="margin-bottom: 10px;"><textarea id="cert" spellcheck="false" name="cert" style="width: 446px; height: 200px; font-family: monospace;" class="form-control"></textarea></div><div style="margin-top: 6px;"><button id="add-ca-cert-popup" class="btn btn-sm btn-primary" disabled><?php echo _("Add Certificate"); ?></button></div></div>';
            ...
            setTimeout(function() {
                ...
                $.post("<?php echo $component_url; ?>/ajax.php", { cmd: "getcertinfo", cert: cert }, function(data) {
                    ...
                    if (data.error) {
                        ...
                        $("#cahost").val(data.certinfo.subject.CN);
        ...
        $.post("<?php echo $component_url; ?>/ajax.php", { cmd: "getcerts" }, function(data) {
            if (data.length > 0) {
                $.each(data, function(k, v) {
                    ...
                    html += '<tr><td><span title="ID: '+v.id+'">'+v.host+'</span></td><td>'+v.issuer+'</td><td>'+to.toString()+'</td><td style="text-align: center;"><a style="font-size: 18px; display: inline-block;" class="remove" title="<?php echo _("Remove certificate"); ?>" data-certid="'+v.id+'"><i class="fa fa-times"></i></a></td></tr>';
                });
            } else {
                html += '<tr><td colspan="9"><?php echo _("You have not added any CA certificates through the web interface"); ?></td></tr>';
            }
            $("#cert-list").html(html);
        }, "json");
    }
