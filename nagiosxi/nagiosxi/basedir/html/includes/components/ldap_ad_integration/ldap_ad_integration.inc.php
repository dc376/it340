...
function ldap_ad_get_associations($server_id) {
    ...
    $user_xml = get_xml_users(array());
    $count = 0;

    // cycle through user list
    foreach($user_xml->user as $user) {
        $meta_auth_server_id = get_user_meta($user->id, "auth_server_id");
...
function create_auth_conn_obj($server_id='')
{
    ...
    if ($conn_method == "ad") {
        ...
        try {
            $ldap_obj = new basicLDAP($ldap_host, $ldap_port, $base_dn);
            return $ldap_obj;
        } catch (Exception $e) {
            return false;
        }
...
{
    global $ad_error;
    $username = $_SESSION['ldap_ad_username'];
    $password = $_SESSION['ldap_ad_password'];
    $server_id = $_SESSION['ldap_ad_server_id'];
