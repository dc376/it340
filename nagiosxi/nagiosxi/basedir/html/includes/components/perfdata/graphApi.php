...
//GET Vars 
$hostname=pnp_convert_object_name(urldecode(grab_request_var("host","")));
$servicename=pnp_convert_object_name(urldecode(grab_request_var("service","")));
$source = grab_request_var("source", 1);
$view=grab_request_var("view", PNP_VIEW_DEFAULT);
$start=intval(grab_request_var("start", ""));
$end=intval(grab_request_var("end", ""));
...
if(!file_exists($rrdfile)) die(_('No performance data available')); 
