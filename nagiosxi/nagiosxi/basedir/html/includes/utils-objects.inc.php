...
function is_authorized_for_object_id($userid, $objectid, $perms = P_READ)
{
    ...
    if (($perms & P_WRITE)) {
        ...
        if (is_authorized_for_all_objects($userid) == true && $config == true)
            return true;
        elseif ($config == true && in_array($objectid, $authids))
            return true;
        else //if we're checking for write perms and we get here, user doesn't have auth for this
            return false;
