...
// Do actual page display/routing
route_request();
...
function show_service_config($error = false, $msg = "")
{
    ...
    if (is_advanced_user() == true) {
        ...
        echo "<p>" . _("Note: You may update basic settings for the service below or use the") . " 
		<a href='" . $url . "' target='_top'>Nagios Core Config Manager</a> " . _("to modify advanced settings for this service.  Service attribute values which are inherited from advanced templates are not shown below.") . "</p>";
    }
    ?>
...
function redirect_service_config($error = false, $msg = "")
{
    ...
    $url = get_base_url() . "includes/components/ccm/xi-index.php?cmd=modify&type=service&id=" . $ccm_id;
...
?>
    ...
    <p style="margin: 20px 0 10px 0;">
        ...
        if (is_advanced_user() == true) {
            //$url = get_base_url() . "includes/components/ccm/xi-index.php";
            //echo _("Use the") . " <a href='" . $url . "' target='_top'>" . _('Nagios Core Config Manager') . "</a> " . _("to modify the settings for this service.");
        } else {
...
function do_config_service($error = false, $msg = "")
{
    ...
    if (isset($new_service["first_notification_delay"]))
        unset($new_service["first_notification_delay"]);
...
?>
    ...
    if (is_advanced_user() == true) {
        $url = get_base_url() . "includes/components/ccm/xi-index.php";
        echo "<p>" . _("Note: You may update basic settings for the host below or use the") . " 
				<a href='" . $url . "' target='_top'>" . _('Nagios Core Config Manager') . "</a> " . _("to modify advanced settings for this host.  Host attribute values which are inherited from advanced templates are not shown below") . ".</p>";
    }
    ?>
    <form method="post" action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>">
    <?php echo get_nagios_session_protector(); ?>
    ...
    <div id="tabs" class="hide">
        ...
        <div id="monitoring-tab">
            ...
            <table class="table table-condensed table-no-border table-auto-width table-no-margin">
                ...
                <?php if (is_advanced_user() == true) { // Advanced user ?>
                    <tr>
                        <td></td>
                    </tr>
                    <tr>
...
function redirect_host_config($error = false, $msg = "")
{
    ...
    <p>
        ...
        if (is_advanced_user() == true) {
            //$url = get_base_url() . "includes/components/ccm/xi-index.php";
            //echo "Use the <a href='" . $url . "' target='_top'>Nagios Core Config Manager</a> " . _("to modify the settings for this host.");
        } else
