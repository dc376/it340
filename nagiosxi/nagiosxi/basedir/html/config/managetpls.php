...
//
// $Id: configobject.php 3716 2015-09-18 18:38:00Z jomann $
...
function draw_edit_page()
{
    ...
    $check_interval = grab_array_var($tpl['tpl'], 'check_interval', $check_interval);
    $retry_interval = grab_array_var($tpl['tpl'], 'retry_interval', $retry_interval);
    $max_check_attempts = grab_array_var($tpl['tpl'], 'max_check_attempts', $max_check_attempts);

    $notification_interval = grab_array_var($tpl['tpl'], 'notification_interval', $notification_interval);
    $first_notification_delay = grab_array_var($tpl['tpl'], 'first_notification_delay', $first_notification_delay);
    $notification_options = grab_array_var($tpl['tpl'], 'notification_options', $notification_options);
    $notification_targets = grab_array_var($tpl['tpl'], 'notification_targets', $notification_targets);
    if (!is_array($notification_targets)) { $notification_targets = unserialize(base64_decode($notification_targets)); }
    $contact_id = grab_array_var($tpl['tpl'], 'contact_id', $contact_id);
    if (!is_array($contact_id)) { $contact_id = unserialize(base64_decode($contact_id)); }
    $contacts = grab_array_var($tpl['tpl'], 'contacts', $contacts);
    if (!is_array($contacts)) { $contacts = unserialize(base64_decode($contacts)); }
    $contactgroup_id = grab_array_var($tpl['tpl'], 'contactgroup_id', $contactgroup_id);
    if (!is_array($contactgroup_id)) { $contactgroup_id = unserialize(base64_decode($contactgroup_id)); }
    $contactgroups = grab_array_var($tpl['tpl'], 'contactgroups', $contactgroups);
    if (!is_array($contactgroups)) { $contactgroups = unserialize(base64_decode($contactgroups)); }

    $parenthost_id = grab_array_var($tpl['tpl'], 'parenthost_id', $parenthost_id);
    if (!is_array($parenthost_id)) { $parenthost_id = unserialize(base64_decode($parenthost_id)); }
    $parenthosts = grab_array_var($tpl['tpl'], 'parenthosts', $parenthosts);
    if (!is_array($parenthosts)) { $parenthosts = unserialize(base64_decode($parenthosts)); }
    $hostgroup_id = grab_array_var($tpl['tpl'], 'hostgroup_id', $hostgroup_id);
    if (!is_array($hostgroup_id)) { $hostgroup_id = unserialize(base64_decode($hostgroup_id)); }
    $hostgroups = grab_array_var($tpl['tpl'], 'hostgroups', $hostgroups);
    if (!is_array($hostgroups)) { $hostgroups = unserialize(base64_decode($hostgroups)); }
    $servicegroup_id = grab_array_var($tpl['tpl'], 'servicegroup_id', $servicegroup_id);
    if (!is_array($servicegroup_id)) { $servicegroup_id = unserialize(base64_decode($servicegroup_id)); }
    $servicegroups = grab_array_var($tpl['tpl'], 'servicegroups', $servicegroups);
    if (!is_array($servicegroups)) { $servicegroups = unserialize(base64_decode($servicegroups)); }
...
function do_update()
{
    ...
    if (!empty($notification_interval)) { $arr['tpl']['notification_interval'] = $notification_interval; }
    if (!empty($first_notification_delay)) { $arr['tpl']['first_notification_delay'] = $first_notification_delay; }
    if (!empty($notification_options)) { $arr['tpl']['notification_options'] = $notification_options; }
    $arr['tpl']['notification_targets'] = base64_encode(serialize($notification_targets));
    $arr['tpl']['contact_id'] = base64_encode(serialize($contact_id));
    $arr['tpl']['contacts'] = base64_encode(serialize($contacts));
    $arr['tpl']['contactgroup_id'] = base64_encode(serialize($contactgroup_id));
    $arr['tpl']['contactgroups'] = base64_encode(serialize($contactgroups));
    ...
    $arr['tpl']['parenthost_id'] = base64_encode(serialize($parenthost_id));
    $arr['tpl']['parenthosts'] = base64_encode(serialize($parenthosts));
    $arr['tpl']['hostgroup_id'] = base64_encode(serialize($hostgroup_id));
    $arr['tpl']['hostgroups'] = base64_encode(serialize($hostgroups));
    $arr['tpl']['servicegroup_id'] = base64_encode(serialize($servicegroup_id));
    $arr['tpl']['servicegroups'] = base64_encode(serialize($servicegroups));
