...
?>
    ...
    <p>
        <?php echo _('Manage the monitoring plugins and scripts that are installed on this system. Use caution when deleting plugins or scripts, as they may cause your monitoring system to generate errors.'); ?><br><?php echo _(' Find thousands of community-developed plugins to extend Nagios XI\'s capabilities at the'); ?> <a href="https://exchange.nagios.org/directory/Plugins" target="_blank" rel="noreferrer"><?php echo _('Nagios Exchange'); ?><i class="fa fa-external-link fa-ml"></i></a>.
    </p>
...
function do_upload()
{
    ...
    //  show_plugins(true,_("No plugin selected for upload."));
    ...
    if (move_uploaded_file($_FILES['uploadedfile']['tmp_name'], $target_path)) {
        ...
        if ($convert_file) {
            $cmd = 'mv '.$target_path.' '.$target_path.'.old; tr -d \'\15\32\' < '.$target_path.'.old > '.$target_path.'; rm -rf '.$target_path.'.old;';
            $out = shell_exec($cmd);
        }
...
function do_delete()
{
    ...
    // log it
    send_to_audit_log(_("User deleted plugin '") . $file . "'", AUDITLOGTYPE_DELETE);
