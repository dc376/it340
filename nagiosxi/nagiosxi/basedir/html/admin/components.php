...
function watch_job()
{
    ...
    <div>
        <?php echo _("Manage the components that are installed on this system.  Need a custom component created to extend Nagios XI's capabilities?  <a href='https://www.nagios.com/contact/' target='_blank'>Contact us</a> for pricing information."); ?>
        <p>
...
function do_delete()
{
    ...
    show_components(false, _("Component scheduled for delettion."));
    exit();
}
...
function do_checkupdates()
{
    $tmp = get_tmp_dir() . "/";
    $xmlcache = $tmp . 'components_api_versions.xml';
    $url = "https://api.nagios.com/product_versions/nagiosxi/5/components_api_versions.xml";
...
function do_configure($error = false, $msg = "")
{
    ...
    // log it
    send_to_audit_log(_("User configured component '") . $component_name . "'", AUDITLOGTYPE_CHANGE);
