    ...
    foreach ($subdirs as $sd) {
        ...
        if (is_dir($d)) {
            ...
            if (file_exists($cf)) {
                ...
                $wizard = current($configwizards);
                
                $wizardhtml .= show_wizard($wizard_dir, $wizard, $x);
...
function watch_job()
{
    ...
    <p>
        <?php echo _("Manage the configuration wizards that are installed on this system and available to users under the <a href='../config/'>configuration</a> menu.  Need a custom configuration wizard created for your organization?  <a href='https://www.nagios.com/contact/' target='_blank'>Contact us</a> for pricing information."); ?>
    ...
    <div class="well" style="margin-top: 10px;">
        <form enctype="multipart/form-data" action="configwizards.php" method="post" style="margin: 0;">
            ...
            <div class="fl" style="height: 29px; line-height: 29px; margin-right: 10px; font-weight: bold; color: #666;"><?php echo _('Upload a Wizard'); ?></div>
            ...
            <div class="fr">
                ...
                <button type="button" class="btn btn-sm btn-success" id="install" style="margin-right: 5px;" <?php if (!configwizard_install_available($wizards, $configwizards_api_versions)) { echo 'disabled'; } ?>><?php echo _("Install Updates"); ?></button>
...
function show_wizard($wizard_dir, $cw, $x)
{
    ...
    if ($img != "") {
        $html .= "<td style='text-align: center;'><img src='" . get_base_url() . "includes/components/nagioscore/ui/images/logos/" . $img . "'></td>";
    }
    $html .= "<td>";
...
function do_checkupdates()
{
    $tmp = get_tmp_dir() . "/";
    $xmlcache = $tmp . 'configwizards_api_versions.xml';
    $url = "https://api.nagios.com/product_versions/nagiosxi/5/configwizards_api_versions.xml";
...
function configwizard_install_available($wizards, $configwizards_api_versions)
{
    $updates = 0;

    foreach ($wizards as $wiz) {
        $name = $wiz['name'];
        if (isset($configwizards_api_versions->{$name})) {
...
function do_cw_install()
{
    ...
    $arr['cmd_id'] = submit_command(COMMAND_UPGRADE_CONFIGWIZARD, $args);
