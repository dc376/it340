...
?>
    ...
    function resize_nav() {
        ...
        if ($('.help-right-nav').outerHeight() > height) {
            $('.help-right-nav').css('height', height);
        } else {
            $('.help-right-nav').css('height', 'auto');
        }
...
$callback_data = array(
    ...
    array(
        ...
        "description" =>            sprintf(_("Executed directly before the %s tag is printed."), htmlentities("</body>")),
        "notes" =>                  "",
    ...
    array(
        ...
        "description" =>            sprintf(_("Executed directly after the %s tag is printed."), htmlentities("</body>")),
        "notes" =>                  "",
        "callback_args" =>          $callback_table
                                        ->reset(_("Array key"), _("Value type"), _("Value description"))
                                        ->tr("child", "boolean", _("False if main window, true if main content (frame)."))
    ...
    array(
        ...
        "description" =>            sprintf(_("Executed directly after %s is printed."), htmlentities('<div id="mainframe">')),
    ...
    array(
        ...
        "description" =>            sprintf(_("Executed after each of the %s tags are written in the host and service status tables, but before the %s tag"), htmlentities("<td>"), htmlentities("</td>")),
        "notes" =>                  sprintf(_("Used when generating host/service status tables. Allows you to insert data to the end of each table row in the status table, BEFORE the %s tag is written."), htmlentities("</tr>")),
        "callback_args" =>          $callback_table
                                        ->reset(_("Array key"), _("Value type"), _("Value description"))
                                        ...
                                        ->tr("table_data", "array", sprintf(_("The array to append elements to, they will be inserted at the end of this cell's row, before the %s."), htmlentities("</tr>")))
                                        ->return_text(),
        "conditional_checks" =>     sprintf(_("If allow_html is true, then only %s and %s tags are allowed (you'll need those to create a table cell)."), htmlentities("<td>"), htmlentities("</td>")),
        "xi_output_changed" =>      "\$callback_arguments['table_data'] = array()",
        ),

    array(
        "title" =>                  "CALLBACK_CUSTOM_TABLE_HEADERS",
        "description" =>            sprintf(_("Executed after each of the %s tags are written in the host and service status tables, but before the %s tag"), htmlentities("<th>"), htmlentities("</tr>")),
        "notes" =>                  sprintf(_("Used when generating host/service status tables. Allows you to insert data to the end of the header table row in the status table, BEFORE the %s tag is written."), htmlentities("</tr>")),
        "callback_args" =>          $callback_table
                                        ->reset(_("Array key"), _("Value type"), _("Value description"))
                                        ...
                                        ->return_text(),
        "conditional_checks" =>     sprintf(_("If allow_html is true, then only %s and %s tags are allowed (you'll need those to create a table header)."), htmlentities("<th>"), htmlentities("</th>")),
        "xi_output_changed" =>      "\$callback_arguments['table_headers'] = array()",
        ),

    array(
        "title" =>                  "CALLBACK_CUSTOM_TABLE_ICONS",
        "description" =>            sprintf(_("Executed right before the %s tag is printed."), htmlentities("<div class='extraicons'>")),
        "notes" =>                  sprintf(_("Used when generating host/service status tables. Allows you to insert data into the icon section of the table cells, inside of %s."), htmlentities("<div class='extraicons'></div>")),
        "callback_args" =>          $callback_table
                                        ->reset(_("Array key"), _("Value type"), _("Value description"))
                                        ->tr("objecttype", "Constant: OBJECTTYPE_SERVICE or OBJECTTYPE_HOST", _("The type of table cell you're dealing with"))
                                        ->tr("host_name", "string", sprintf(_("If %s then this is the host name of the object that this cell refers to, if %s then this is the host name of the service_description of the object that this cell refers to."), "OBJECTTYPE_HOST", "OBJECTTYPE_SERVICE"))
                                        ...
                                        ->tr("object_data", "SimpleXML", _("The XML of the object that this cell refers to."))
                                        ->tr("allow_html", "boolean", _("Value is set to inform whether or not HTML is allowed in the status table."))
                                        ->tr("icons", "array", sprintf(_("The array to append elements to, they will be inserted into this cell's %s section."), htmlentities("<div class='extraicons'></div>")))
                                        ->return_text(),
        "conditional_checks" =>     sprintf(_("If allow_html is true, then only %s tags are allowed (you'll need that to display your icon)."), htmlentities("<img>")),
    ...
    array(
        ...
        "description" =>            sprintf(_("Executed after the footer %s is printed, but before the page %s is printed."), htmlentities("</div>"), htmlentities("</div>")),
        "notes" =>                  "",
        "callback_args" =>          $callback_table
                                        ->reset(_("Array key"), _("Value type"), _("Value description"))
                                        ->tr("child", "boolean", _("False if main window, true if main content (frame)."))
    ...
    array(
        ...
        "description" =>            sprintf(_("Executed before the header %s is printed."), htmlentities("</div>")),
        "notes" =>                  "",
        "callback_args" =>          $callback_table
                                        ->reset(_("Array key"), _("Value type"), _("Value description"))
                                        ->tr("child", "boolean", _("False if main window, true if main content (frame)."))
    ...
    array(
        ...
        "callback_args" =>          $callback_table
                                        ...
                                        ->tr("page_url", "string", sprintf(_("The string used to redirect if %s."), htmlentities("redirect_url == true")))
    ...
    array(
        ...
        "notes" =>                  sprintf(_("Although not necessary, in order to maintain consistency, the preferred format for the actions array elements is: %s"), htmlentities("<li><div class='commandimage'><img></div><div class='commandtext'><a href=\$actionlink>\$actiontext</a></div></li>")),
        "callback_args" =>          $callback_table
                                        ->reset(_("Array key"), _("Value type"), _("Value description"))
                                        ...
                                        ->return_text(),
        "conditional_checks" =>     "",
        "xi_output_changed" =>      sprintf(_("The Quick Actions list on Host Status Detail pages will be affected by the elements appended to the %s array"), htmlentities("\$cbargs['actions']")),
        ),
    ...
    array(
        ...
        "callback_args" =>          $callback_table
                                        ...
                                        ->tr("tabs", "array", sprintf(_("The array to append tab information to.%sEach tab to be appended is an array in the format of %s"), "<br>", htmlentities("array('id' => string, 'title' => string, 'content' => string, 'icon' => string)")))
                                        ->return_text(),
        "conditional_checks" =>     "",
        "xi_output_changed" =>      sprintf(_("New tabs will appear on host status pages based on the %s array"), htmlentities("\$cbargs['tabs']")),
        ),
    ...
    array(
        ...
        "callback_args" =>          sprintf(_("This callback actually returns the menu array as %s. The array is in the format of %s"), 
            ...
            "</pre>"),
        "conditional_checks" =>     "",
        "xi_output_changed" =>      sprintf(_("The Nagios XI menus will be altered depending on your removal or addition of menu items from the %s"), htmlentities("\$callback_args.")),
        ),
    ...
    array(
        ...
        "description" =>            sprintf(_("Executed directly before the %s tag is printed."), htmlentities("</head>")),
        "notes" =>                  "",
        "callback_args" =>          $callback_table
                                        ->reset(_("Array key"), _("Value type"), _("Value description"))
                                        ->tr("child", "boolean", _("False if main window, true if main content (frame)."))
    ...
    array(
        ...
        "callback_args" =>          $callback_table
                                        ->reset(_("Array key"), _("Value type"), _("Value description"))
                                        ->tr("credentials", "array", sprintf(_("The array containing the user submitted information, in the form of %s"), "array('username' => string, 'password' => string)"))
                                        ...
                                        ->return_text(),
        "conditional_checks" =>     _("Nagios XI will authenticate a user based on the value of "), htmlentities("\$cbargs['login_ok']"),
        "xi_output_changed" =>      "",
        ),

    array(
        "title" =>                  "CALLBACK_REPORTS_ACTION_LINK",
        "description" =>            sprintf(_("Executed directly after %s is printed on report pages."), htmlentities("<div class='reportexportlinks'>")),
        ...
        "callback_args" =>          $callback_table
                                        ...
                                        ->return_text(),
        "conditional_checks" =>     "",
        "xi_output_changed" =>      sprintf(_("An action link will appear on the reports page depending on elements appended to the %s array."), htmlentities("\$cbargs['actions']")),
        ),
    ...
    array(
        ...
        "callback_args" =>          $callback_table
                                        ...
                                        ->return_text(),
        "conditional_checks" =>     "",
        "xi_output_changed" =>      sprintf(_("The Quick Actions list on Service Status Detail pages will be affected by the elements appended to the %s array"), htmlentities("\$cbargs['actions']")),
        ),
    ...
    array(
        ...
        "callback_args" =>          $callback_table
                                        ...
                                        ->return_text(),
        "conditional_checks" =>     "",
        "xi_output_changed" =>      sprintf(_("New tabs will appear on service status pages based on the %s array"), htmlentities("\$cbargs['tabs']")),
        ),
    ...
    array(
        ...
        "callback_args" =>          $callback_table
                                        ...
                                        ->return_text(),
        "conditional_checks" =>     "",
        "xi_output_changed" =>      sprintf(_("New tabs will appear on the Account/Notification Messages page based on the %s array"), htmlentities("\$cbargs['tabs']")),
        ),
    ...
    array(
        ...
        "callback_args" =>          $callback_table
                                        ...
                                        ->return_text(),
        "conditional_checks" =>     "",
        "xi_output_changed" =>      sprintf(_("New tabs will appear on the Account/Notification Methods page based on the %s array"), htmlentities("\$cbargs['tabs']")),
        ),
