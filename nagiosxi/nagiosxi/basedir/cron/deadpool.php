...
//disable until production ready
if(!use_2014_features()){
	echo "Deadpool is disabled\n"; 
	exit(0); 
}
...
function do_deadpool_reaper() {
	...
	foreach($host_candidates as $h) {
		...
		if ($stage == 1) {
			...
			$cmd = "cd " . get_root_dir() . "/scripts && ./nagiosql_delete_service.php --config=" . $name . "";
			echo "COMMAND: $cmd\n";
			if ($testing == false) {
			...
			// delete host...
			echo "Deleting host...\n";
			$cmd = "cd " . get_root_dir() . "/scripts && ./nagiosql_delete_host.php --id=" . $nagiosql_host_id . "";
			echo "COMMAND: $cmd\n";
	...
	foreach($service_candidates as $s) {
		...
		if ($stage == 1) {
			...
			$cmd = "cd " . get_root_dir() . "/scripts && ./nagiosql_delete_service.php --id=" . $nagiosql_service_id . "";
			echo "COMMAND: $cmd\n";
			if ($testing == false)
				exec($cmd);
...
function send_deadpool_email($hosts, $services) {
	...
	if ($stage1_host_body != "") {
		...
		$fullbody .= "===\n";
		$fullbody .= _("The following hosts were moved to the host deadpool because they have remained in a problem state longer than the stage 1 deadpool threshold.  If the hosts do not recover before the stage 2 threshold, they will automatically be deleted from the monitoring configuration.") . "\n\n";
	...
	if ($stage1_service_body != "") {
		...
		$fullbody .= "===\n";
		$fullbody .= _("The following services were moved to the service deadpool because they have remained in a problem state longer than the stage 1 deadpool threshold.  If the services do not recover before the stage 2 threshold, they will automatically be deleted from the monitoring configuration.") . "\n\n";
	...
	if ($stage2_host_body != "") {
		...
		$fullbody .= "===\n";
		$fullbody .= _("The following hosts were deleted from the monitoring configuration because they remained in a problem state longer than the stage 2 deadpool threshold.") . "\n\n";
	...
	if ($stage2_service_body!="") {
		...
		$fullbody .="===\n";
		$fullbody .= _("The following services were deleted from the monitoring configuration because they remained in a problem state longer than the stage 2 deadpool threshold.") . "\n\n";
