...
function main() {
	...
	$realURL = exec(get_root_dir() . "/tools/geturlas.php --user='$user' --url='$reportURL'");
	$realURL = trim($realURL);
	exec("wget -O \"$reportdir/$filename\" '$realURL&mode=pdf'");

	// Use this for debug output in PHPmailer log
	$debugmsg = "";

	// Set where email is coming from for PHPmailer log
	$send_mail_referer = "tools/getrep.php > PDF Report Email";
